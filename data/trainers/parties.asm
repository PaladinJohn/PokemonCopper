; Trainer data structure:
; - db "NAME@", TRAINERTYPE_* constants |ed together
; - 1 to 6 Pokémon:
;    * in all cases:              db level, species
;    * with TRAINERTYPE_DVS:      db atk|def dv, spd|spc dv
;    * with TRAINERTYPE_STAT_EXP: dw hp, atk, def, spd, spc
;    * with TRAINERTYPE_ITEM:     db item
;    * with TRAINERTYPE_MOVES:    db move 1, move 2, move 3, move 4
;    (TRAINERTYPE_ITEM_MOVES is just TRAINERTYPE_ITEM | TRAINERTYPE_MOVES)
; - db -1 ; end

SECTION "Enemy Trainer Parties 1", ROMX

EmeryGroup:
	; EMERY (1)
	db "EMERY@", TRAINERTYPE_MOVES
	db  10, ROCKRUFF,   TACKLE, LEER, SWAGGER, NO_MOVE
	db  12, RHYHORN,   HORN_ATTACK, TAIL_WHIP, SANDSTORM, PURSUIT
	db -1 ; end

WhitneyGroup:
	; WHITNEY (1)
	db "WHITNEY@", TRAINERTYPE_ITEM_MOVES
	db 18, SANDSHREW,   NO_ITEM,      DIG, METAL_CLAW, SANDSTORM, ATTRACT
	db 18, PHANPY,      BERRY,        ANCIENTPOWER, BODY_SLAM, FOCUS_ENERGY, ROLLOUT
	db 20, STEELIX,     BERRY,        MAGNITUDE, SANDSTORM, ATTRACT, ROCK_SLIDE
	db -1 ; end

BugsyGroup:
	; BUGSY (1)
	db "BUGSY@", TRAINERTYPE_MOVES
	db 15, SMOLIV,     FLAIL, ABSORB, GROWTH, RAZOR_LEAF
	db 15, BELLSPROUT, VINE_WHIP, GROWTH, WRAP, SLEEP_POWDER
	db 17, SUNFLORA,   POUND, GROWTH, RAZOR_LEAF, GIGA_DRAIN
	db -1 ; end

MortyGroup:
	; MORTY (1)
	db "MORTY@", TRAINERTYPE_ITEM_MOVES
	db 21, BOMBIRDIER, NO_ITEM,      PECK, LEER, WING_ATTACK, NO_MOVE
	db 21, SPIRITOMB,  NO_ITEM,      DESTINY_BOND, PURSUIT, CONFUSE_RAY, SHADOW_BALL
	db 25, HOUNDOOM,   BERRY,        CRUNCH, FIRE_SPIN, SMOG, ROAR
	db 23, UMBREON,    BERRY,        CHARM, SHADOW_BALL, SAND_ATTACK, CRUNCH
	db -1 ; end

PryceGroup:
	; PRYCE (1)
	db "PRYCE@", TRAINERTYPE_DVS | TRAINERTYPE_STAT_EXP | TRAINERTYPE_ITEM_MOVES
	db 31, DRAGONAIR
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BERRY
		db THUNDER, THUNDER_WAVE, TWISTER, SLAM
	db 31, AMPHAROS
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BERRY
		db RAIN_DANCE, THUNDERPUNCH, THUNDER_WAVE, COTTON_SPORE
	db 31, JOLTEON
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BERRY
		db SHADOW_BALL, DOUBLE_KICK, THUNDERSHOCK, QUICK_ATTACK
	db 36, LUXRAY
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BERRY
		db THUNDER, RAIN_DANCE, CRUNCH, SPARK
	db -1 ; end

JasmineGroup:
	; JASMINE (1)
	db "JASMINE@", TRAINERTYPE_DVS | TRAINERTYPE_STAT_EXP | TRAINERTYPE_ITEM_MOVES
	db 27, SWINUB
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BERRY
		db ICE_BEAM, ENDURE, TAKE_DOWN, HORN_ATTACK
	db 27, CROBAT
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db PRZCUREBERRY
		db HAZE, GIGA_DRAIN, CONFUSE_RAY, BITE
	db 29, FROSMOTH
		db $7f, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BITTER_BERRY
		db ICE_BEAM, MIRROR_COAT, ATTRACT, STRUGGLE_BUG
	db 31, LAPRAS
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db PRZCUREBERRY
		db ICE_BEAM, SING, HYDRO_PUMP, BODY_SLAM
	db -1 ; end

ChuckGroup:
	; CHUCK (1)
	db "CHUCK@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_ITEM_MOVES
	db 27, CHARMELEON
	    dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BITTER_BERRY
		db CRUNCH, FIRE_BLAST, IRON_TAIL, OUTRAGE
	db 27, SKARMORY
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BERRY
		db DRILL_PECK, PURSUIT, SWIFT, STEEL_WING
	db 27, DURANT
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BITTER_BERRY
		db STRENGTH, FURY_CUTTER, CRUNCH, METAL_CLAW
	db 30, KINGLER
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BERRY
		db STRENGTH, MUD_SHOT, METAL_CLAW, SWAGGER
	db -1 ; end

ClairGroup:
	; CLAIR (1)
	db "CLAIR@", TRAINERTYPE_DVS | TRAINERTYPE_STAT_EXP | TRAINERTYPE_ITEM_MOVES
	db 38, HOUNDSTONE
		db $11, $11
	    dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db PINK_BOW
		db REST, SLEEP_TALK, LICK, HEADBUTT
	db 38, ALAKAZAM
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db TWISTEDSPOON
		db PSYCHIC_M, SHADOW_BALL, RECOVER, PSYBEAM
	db 38, STARMIE
		db $11, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db MYSTIC_WATER
		db PSYCHIC_M, SURF, CONFUSE_RAY, MINIMIZE
	db 41, ESPEON
		db $fd, $11
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db TWISTEDSPOON
		db PSYCHIC_M, SHADOW_BALL, HIDDEN_POWER, SAND_ATTACK
	db -1 ; end

Rival1Group:
	; RIVAL1 (1)
	db "?@", TRAINERTYPE_DVS
	db  5, BULBASAUR, $dd, $dd
	db -1 ; end

	; RIVAL1 (2)
	db "?@", TRAINERTYPE_DVS
	db  5, CHARMANDER, $dd, $dd
	db -1 ; end

	; RIVAL1 (3)
	db "?@", TRAINERTYPE_DVS
	db  5, SQUIRTLE, $dd, $dd
	db -1 ; end

	; RIVAL1 (4)
	db "?@", TRAINERTYPE_DVS
	db 13, ROOKIDEE, $ed, $dd
	db 15, PAWMI, $ed, $dd
	db 17, IVYSAUR, $ed, $dd
	db -1 ; end

	; RIVAL1 (5)
	db "?@", TRAINERTYPE_DVS
	db 13, ROOKIDEE, $ed, $dd
	db 15, PAWMI, $ed, $dd
	db 17, CHARMELEON, $ed, $dd
	db -1 ; end

	; RIVAL1 (6)
	db "?@", TRAINERTYPE_DVS
	db 13, ROOKIDEE, $ed, $dd
	db 15, PAWMI, $ed, $dd
	db 17, WARTORTLE, $ed, $dd
	db -1 ; end

	; RIVAL1 (7)
	db "?@", TRAINERTYPE_DVS | TRAINERTYPE_MOVES
	db 20, CORVISQUIR
		db $ee, $dd
		db WING_ATTACK, LEER, SHARPEN, FURY_ATTACK
	db 18, SCYTHER
		db $ee, $dd
		db QUICK_ATTACK, LEER, FOCUS_ENERGY, PURSUIT
	db 20, PAWMO
		db $ee, $dd
		db SCRATCH, GROWL, THUNDERSHOCK, QUICK_ATTACK
	db 22, IVYSAUR
		db $ee, $dd
		db TACKLE, LEECH_SEED, SLEEP_POWDER, RAZOR_LEAF
	db -1 ; end

	; RIVAL1 (8)
	db "?@", TRAINERTYPE_DVS | TRAINERTYPE_MOVES
	db 20, CORVISQUIR
		db $ee, $dd
		db WING_ATTACK, LEER, SHARPEN, FURY_ATTACK
	db 18, SCYTHER
		db $ee, $dd
		db QUICK_ATTACK, LEER, FOCUS_ENERGY, PURSUIT
	db 20, PAWMO
		db $ee, $dd
		db SCRATCH, GROWL, THUNDERSHOCK, QUICK_ATTACK
	db 22, CHARMELEON
		db $ee, $dd
		db SCRATCH, EMBER, METAL_CLAW, SMOKESCREEN
	db -1 ; end

	; RIVAL1 (9)
	db "?@", TRAINERTYPE_DVS | TRAINERTYPE_MOVES
	db 20, CORVISQUIR
		db $ee, $dd
		db WING_ATTACK, LEER, SHARPEN, FURY_ATTACK
	db 18, SCYTHER
		db $ee, $dd
		db QUICK_ATTACK, LEER, FOCUS_ENERGY, PURSUIT
	db 20, PAWMO
		db $ee, $dd
		db SCRATCH, GROWL, THUNDERSHOCK, QUICK_ATTACK
	db 22, WARTORTLE
		db $ee, $dd
		db TACKLE, TAIL_WHIP, WATER_GUN, BITE
	db -1 ; end

	; RIVAL1 (10)
	db "?@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_MOVES
	db 36, CORVISQUIR
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db SHARPEN, FURY_ATTACK, SAND_ATTACK, DRILL_PECK
	db 34, SCIZOR
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db QUICK_ATTACK, PURSUIT, WING_ATTACK, METAL_CLAW
	db 36, PAWMOT
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db ICE_PUNCH, THUNDERSHOCK, QUICK_ATTACK, NUZZLE
	db 38, UMBREON
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db SAND_ATTACK, QUICK_ATTACK, CONFUSE_RAY, FAINT_ATTACK
	db 38, VENUSAUR
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db TACKLE, LEECH_SEED, SLEEP_POWDER, RAZOR_LEAF
	db -1 ; end

	; RIVAL1 (11)
	db "?@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_MOVES
	db 36, CORVISQUIR
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db SHARPEN, FURY_ATTACK, SAND_ATTACK, DRILL_PECK
	db 34, SCIZOR
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db QUICK_ATTACK, PURSUIT, WING_ATTACK, METAL_CLAW
	db 36, PAWMOT
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db ICE_PUNCH, THUNDERSHOCK, QUICK_ATTACK, NUZZLE
	db 38, UMBREON
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db SAND_ATTACK, QUICK_ATTACK, CONFUSE_RAY, FAINT_ATTACK
	db 38, CHARIZARD
		dw $00fb, $010b, $00f3, $00fe0, $00c2 ; hp, atk, def, spd, spc
		db METAL_CLAW, SMOKESCREEN, FLAMETHROWER, WING_ATTACK
	db -1 ; end

	; RIVAL1 (12)
	db "?@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_MOVES
	db 36, CORVISQUIR
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db SHARPEN, FURY_ATTACK, SAND_ATTACK, DRILL_PECK
	db 34, SCIZOR
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db QUICK_ATTACK, PURSUIT, WING_ATTACK, METAL_CLAW
	db 36, PAWMOT
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db ICE_PUNCH, THUNDERSHOCK, QUICK_ATTACK, NUZZLE
	db 38, UMBREON
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db SAND_ATTACK, QUICK_ATTACK, CONFUSE_RAY, FAINT_ATTACK
	db 38, BLASTOISE
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db TACKLE, TAIL_WHIP, WATER_GUN, BITE
	db -1 ; end

	; RIVAL1 (13)
	db "?@", TRAINERTYPE_MOVES
	db 34, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 36, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 35, BOLTUND,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 35, HAUNTER,    MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 35, KADABRA,    DISABLE, PSYBEAM, RECOVER, FUTURE_SIGHT
	db 38, MEOWSCARAD,   REFLECT, RAZOR_LEAF, POISONPOWDER, BODY_SLAM
	db -1 ; end

	; RIVAL1 (14)
	db "?@", TRAINERTYPE_MOVES
	db 34, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 36, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 35, BOLTUND,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 35, HAUNTER,    MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 35, KADABRA,    DISABLE, PSYBEAM, RECOVER, FUTURE_SIGHT
	db 38, TYPHLOSION, SMOKESCREEN, EMBER, QUICK_ATTACK, FLAME_WHEEL
	db -1 ; end

	; RIVAL1 (15)
	db "?@", TRAINERTYPE_MOVES
	db 34, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 36, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 34, BOLTUND,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 35, HAUNTER,    MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 35, KADABRA,    DISABLE, PSYBEAM, RECOVER, FUTURE_SIGHT
	db 38, FERALIGATR, RAGE, WATER_GUN, SCARY_FACE, SLASH
	db -1 ; end

PokemonProfGroup:

WillGroup:
	; WILL (1)
	db "WILL@", TRAINERTYPE_MOVES
	db 40, XATU,       QUICK_ATTACK, FUTURE_SIGHT, CONFUSE_RAY, PSYCHIC_M
	db 41, CORVIKNGHT,       DOUBLESLAP, NUZZLE, ICE_PUNCH, PSYCHIC_M
	db 41, LUCARIO,    REFLECT, LEECH_SEED, EGG_BOMB, PSYCHIC_M
	db 41, SLOWBRO,    CURSE, AMNESIA, BODY_SLAM, PSYCHIC_M
	db 42, XATU,       QUICK_ATTACK, FUTURE_SIGHT, CONFUSE_RAY, PSYCHIC_M
	db -1 ; end

PKMNTrainerGroup:
	; CAL (1)
	db "CAL@", TRAINERTYPE_NORMAL
	db 10, SPRIGATITO
	db 10, CYNDAQUIL
	db 10, TOTODILE
	db -1 ; end

	; CAL (2)
	db "CAL@", TRAINERTYPE_NORMAL
	db 30, FLORAGATO
	db 30, QUILAVA
	db 30, CROCONAW
	db -1 ; end

	; CAL (3)
	db "CAL@", TRAINERTYPE_NORMAL
	db 50, MEOWSCARAD
	db 50, TYPHLOSION
	db 50, FERALIGATR
	db -1 ; end

BrunoGroup:
	; BRUNO (1)
	db "BRUNO@", TRAINERTYPE_MOVES
	db 42, SPIRITOMB,  PURSUIT, QUICK_ATTACK, DIG, DETECT
	db 42, GULPIN,  SWAGGER, DOUBLE_KICK, HI_JUMP_KICK, FORESIGHT
	db 42, SIGILYPH, THUNDERPUNCH, ICE_PUNCH, FIRE_PUNCH, YAWN
	db 43, ONIX,       BIND, EARTHQUAKE, SANDSTORM, ROCK_SLIDE
	db 46, MACHAMP,    ROCK_SLIDE, FORESIGHT, VITAL_THROW, CROSS_CHOP
	db -1 ; end

KarenGroup:
	; KAREN (1)
	db "KAREN@", TRAINERTYPE_MOVES
	db 42, UMBREON,    SAND_ATTACK, CONFUSE_RAY, FAINT_ATTACK, MEAN_LOOK
	db 42, VILEPLUME,  STUN_SPORE, ACID, MOONLIGHT, PETAL_DANCE
	db 45, GENGAR,     LICK, SPITE, CURSE, DESTINY_BOND
	db 44, BOMBIRDIER,    QUICK_ATTACK, WHIRLWIND, PURSUIT, FAINT_ATTACK
	db 47, HOUNDOOM,   ROAR, PURSUIT, FLAMETHROWER, CRUNCH
	db -1 ; end

KogaGroup:
	; KOGA (1)
	db "KOGA@", TRAINERTYPE_MOVES
	db 40, SWALOT,    DOUBLE_TEAM, TACKLE, BATON_PASS, GIGA_DRAIN
	db 41, FROSMOTH,   SUPERSONIC, GUST, PSYCHIC_M, TOXIC
	db 43, BEWEAR, PROTECT, SWIFT, EXPLOSION, SPIKES
	db 42, MUK,        MINIMIZE, ACID_ARMOR, SLUDGE_BOMB, TOXIC
	db 44, CROBAT,     DOUBLE_TEAM, QUICK_ATTACK, WING_ATTACK, TOXIC
	db -1 ; end

ChampionGroup:
	; CHAMPION (1)
	db "LANCE@", TRAINERTYPE_MOVES
	db 44, GYARADOS,   FLAIL, RAIN_DANCE, SURF, HYPER_BEAM
	db 47, DRAGONITE,  THUNDER_WAVE, TWISTER, THUNDER, HYPER_BEAM
	db 47, DRAGONITE,  THUNDER_WAVE, TWISTER, BLIZZARD, HYPER_BEAM
	db 46, AERODACTYL, WING_ATTACK, ANCIENTPOWER, ROCK_SLIDE, HYPER_BEAM
	db 46, CHARIZARD,  FLAMETHROWER, WING_ATTACK, SLASH, HYPER_BEAM
	db 50, DRAGONITE,  FIRE_BLAST, SAFEGUARD, OUTRAGE, HYPER_BEAM
	db -1 ; end

BrockGroup:
	; BROCK (1)
	db "BROCK@", TRAINERTYPE_MOVES
	db 41, GRAVELER,   DEFENSE_CURL, ROCK_SLIDE, ROLLOUT, EARTHQUAKE
	db 41, RHYHORN,    FURY_ATTACK, SCARY_FACE, EARTHQUAKE, HORN_DRILL
	db 42, CARRACOSTA,    BITE, SURF, PROTECT, SPIKE_CANNON
	db 44, ONIX,       BIND, ROCK_SLIDE, BIDE, SANDSTORM
	db 42, KABUTOPS,   SLASH, SURF, ENDURE, GIGA_DRAIN
	db -1 ; end

MistyGroup:
	; MISTY (1)
	db "MISTY@", TRAINERTYPE_MOVES
	db 42, GOLDUCK,    SURF, DISABLE, PSYCH_UP, PSYCHIC_M
	db 42, QUAGSIRE,   SURF, AMNESIA, EARTHQUAKE, RAIN_DANCE
	db 44, LAPRAS,     SURF, PERISH_SONG, BLIZZARD, RAIN_DANCE
	db 47, STARMIE,    SURF, CONFUSE_RAY, RECOVER, ICE_BEAM
	db -1 ; end

LtSurgeGroup:
	; LT_SURGE (1)
	db "LT.SURGE@", TRAINERTYPE_MOVES
	db 44, LUXRAY,     THUNDER_WAVE, QUICK_ATTACK, THUNDERBOLT, THUNDER
	db 40, ZEBSTRIKA,  SCREECH, DOUBLE_TEAM, SWIFT, EXPLOSION
	db 40, BOLTUND,   LOCK_ON, DOUBLE_TEAM, SWIFT, ZAP_CANNON
	db 40, ZEBSTRIKA,  SCREECH, DOUBLE_TEAM, SWIFT, EXPLOSION
	db 46, WHIRLIPEDE, QUICK_ATTACK, THUNDERPUNCH, LIGHT_SCREEN, THUNDER
	db -1 ; end

ScientistGroup:
	; SCIENTIST (1)
	db "MALCOLM@", TRAINERTYPE_NORMAL
	db 30, KOFFING
	db 30, KOFFING
	db -1 ; end

	; SCIENTIST (2)
	db "MURRAY@", TRAINERTYPE_NORMAL
	db 27, KOFFING
	db 32, DITTO
	db -1 ; end

	; SCIENTIST (3)
	db "SAMUEL@", TRAINERTYPE_NORMAL
	db 28, KADABRA
	db 28, KADABRA
	db 28, DITTO
	db -1 ; end

	; SCIENTIST (4)
	db "GREIG@", TRAINERTYPE_NORMAL
	db 33, KADABRA
	db 33, HYPNO
	db 33, GRIMER
	db -1 ; end

	; SCIENTIST (5)
	db "FRANCIS@", TRAINERTYPE_MOVES
	db 30, KOFFING,    TACKLE, SELFDESTRUCT, SLUDGE, SMOKESCREEN
	db 36, PORYGON,    PSYBEAM, RECOVER, SHARPEN, TRI_ATTACK
	db -1 ; end

ErikaGroup:
	; ERIKA (1)
	db "ERIKA@", TRAINERTYPE_MOVES
	db 42, TANGELA,    VINE_WHIP, BIND, GIGA_DRAIN, SLEEP_POWDER
	db 41, ARBOLIVA,   MEGA_DRAIN, LEECH_SEED, COTTON_SPORE, GIGA_DRAIN
	db 46, VICTREEBEL, SUNNY_DAY, SYNTHESIS, ACID, RAZOR_LEAF
	db 46, BELLOSSOM,  SUNNY_DAY, SYNTHESIS, PETAL_DANCE, SOLARBEAM
	db -1 ; end

YoungsterGroup:
	; YOUNGSTER (1)
	db "JOEY@", TRAINERTYPE_NORMAL
	db  6, RATTATA
	db -1 ; end

	; YOUNGSTER (2)
	db "CRISTOPH@", TRAINERTYPE_NORMAL
	db  4, SENTRET
	db  6, HOOTHOOT
	db -1 ; end

	; YOUNGSTER (3)
	db "STEVEN@", TRAINERTYPE_NORMAL
	db  8, FLETCHLING
	db  10, SMOLIV
	db -1 ; end

	; YOUNGSTER (4)
	db "ROBERT@", TRAINERTYPE_NORMAL
	db  7, RATTATA
	db 12, SPEAROW
	db -1 ; end

	; YOUNGSTER (5)
	db "RYAN@", TRAINERTYPE_NORMAL
	db 12, GEODUDE
	db 12, PHANPY
	db 12, ZUBAT
	db 12, HOOTHOOT
	db -1 ; end

	; YOUNGSTER (6)
	db "GRAEME@", TRAINERTYPE_NORMAL
	db 12, LEDYBA
	db 12, SHINX
	db -1 ; end

	; YOUNGSTER (7)
	db "JOEY@", TRAINERTYPE_NORMAL
	db 17, RATTATA
	db -1 ; end

	; YOUNGSTER (8)
	db "JOEY@", TRAINERTYPE_MOVES
	db 24, RATICATE,   TAIL_WHIP, QUICK_ATTACK, HYPER_FANG, FOCUS_ENERGY
	db -1 ; end

	; YOUNGSTER (9)
	db "WARREN@", TRAINERTYPE_NORMAL
	db 35, FEAROW
	db -1 ; end

	; YOUNGSTER (10)
	db "JIMMY@", TRAINERTYPE_NORMAL
	db 33, RATICATE
	db 33, ZOROARK
	db -1 ; end

	; YOUNGSTER (11)
	db "OWEN@", TRAINERTYPE_NORMAL
	db 35, GROWLITHE
	db -1 ; end

	; YOUNGSTER (12)
	db "JASON@", TRAINERTYPE_NORMAL
	db 33, SANDSLASH
	db 33, CROBAT
	db -1 ; end

	; YOUNGSTER (13)
	db "DAVID@", TRAINERTYPE_MOVES
	db 30, RATICATE,   TAIL_WHIP, QUICK_ATTACK, HYPER_FANG, PURSUIT
	db -1 ; end

	; YOUNGSTER (14)
	db "DAVID@", TRAINERTYPE_MOVES
	db 37, RATICATE,   HYPER_BEAM, QUICK_ATTACK, HYPER_FANG, PURSUIT
	db -1 ; end
	
	; YOUNGSTER (15)
	db "ALEX@", TRAINERTYPE_NORMAL
	db 13, SWINUB
	db 13, SANDSHREW
	db 17, GEODUDE
	db -1 ; end

SchoolboyGroup:
	; SCHOOLBOY (1)
	db "JONATHAN@", TRAINERTYPE_NORMAL
	db 17, ODDISH
	db 20, DITTO
	db -1 ; end

	; SCHOOLBOY (2)
	db "KIPP@", TRAINERTYPE_NORMAL
	db 27, BLITZLE
	db 27, YAMPER
	db 31, BLITZLE
	db 31, BOLTUND
	db -1 ; end

	; SCHOOLBOY (3)
	db "ALLAN@", TRAINERTYPE_NORMAL
	db 16, ODDISH
	db 21, TANGELA
	db -1 ; end

	; SCHOOLBOY (4)
	db "JOHNNY@", TRAINERTYPE_NORMAL
	db 29, BELLSPROUT
	db 31, WEEPINBELL
	db 33, VICTREEBEL
	db -1 ; end

	; SCHOOLBOY (5)
	db "DANNY@", TRAINERTYPE_NORMAL
	db 31, CORVIKNGHT
	db 31, WHIRLIPEDE
	db 31, MAGMAR
	db -1 ; end

	; SCHOOLBOY (6)
	db "TOMMY@", TRAINERTYPE_NORMAL
	db 32, XATU
	db 34, ALAKAZAM
	db -1 ; end

	; SCHOOLBOY (7)
	db "DUDLEY@", TRAINERTYPE_NORMAL
	db 35, ODDISH
	db -1 ; end

	; SCHOOLBOY (8)
	db "JOE@", TRAINERTYPE_NORMAL
	db 33, TANGELA
	db 33, VAPOREON
	db -1 ; end

	; SCHOOLBOY (9)
	db "BILLY@", TRAINERTYPE_NORMAL
	db 27, PARAS
	db 27, PARAS
	db 27, LYCANROC
	db 35, DITTO
	db -1 ; end

	; SCHOOLBOY (10)
	db "GRANT@", TRAINERTYPE_MOVES
	db 19, BELLSPROUT,  VINE_WHIP, GROWTH, WRAP, SLEEP_POWDER
	db 24, QUAGSIRE,    WATER_GUN, TAIL_WHIP, SLAM, AMNESIA
	db -1 ; end

	; SCHOOLBOY (11)
	db "NATE@", TRAINERTYPE_NORMAL
	db 32, LEDIAN
	db 32, LUCARIO
	db -1 ; end

	; SCHOOLBOY (12)
	db "RICKY@", TRAINERTYPE_NORMAL
	db 32, AIPOM
	db 32, DITTO
	db -1 ; end

	; SCHOOLBOY (13)
	db "JACK@", TRAINERTYPE_NORMAL
	db 14, ODDISH
	db 17, BLITZLE
	db -1 ; end

	; SCHOOLBOY (14)
	db "JACK@", TRAINERTYPE_NORMAL
	db 28, GLOOM
	db 31, ZEBSTRIKA
	db -1 ; end

	; SCHOOLBOY (15)
	db "ALAN@", TRAINERTYPE_NORMAL
	db 17, TANGELA
	db 17, ABSOL
	db -1 ; end

	; SCHOOLBOY (16)
	db "ALAN@", TRAINERTYPE_NORMAL
	db 20, NATU
	db 22, TANGELA
	db 20, QUAGSIRE
	db 25, ABSOL
	db -1 ; end

	; SCHOOLBOY (17)
	db "CHAD@", TRAINERTYPE_NORMAL
	db 19, CAPSAKID
	db 19, YAMPER
	db -1 ; end

	; SCHOOLBOY (18)
	db "CHAD@", TRAINERTYPE_NORMAL
	db 27, CAPSAKID
	db 31, BOLTUND
	db -1 ; end

	; SCHOOLBOY (19)
	db "JACK@", TRAINERTYPE_NORMAL
	db 30, GLOOM
	db 33, GROWLITHE
	db 33, ZEBSTRIKA
	db -1 ; end

	; SCHOOLBOY (20)
	db "JACK@", TRAINERTYPE_MOVES
	db 35, ZEBSTRIKA,  SCREECH, SONICBOOM, ROLLOUT, LIGHT_SCREEN
	db 35, GROWLITHE,  SUNNY_DAY, LEER, TAKE_DOWN, FLAME_WHEEL
	db 37, VILEPLUME,  SOLARBEAM, SLEEP_POWDER, ACID, MOONLIGHT
	db -1 ; end

	; SCHOOLBOY (21)
	db "ALAN@", TRAINERTYPE_NORMAL
	db 27, NATU
	db 27, TANGELA
	db 30, QUAGSIRE
	db 30, ABSOL
	db -1 ; end

	; SCHOOLBOY (22)
	db "ALAN@", TRAINERTYPE_MOVES
	db 35, XATU,       PECK, NIGHT_SHADE, SWIFT, FUTURE_SIGHT
	db 32, TANGELA,    POISONPOWDER, VINE_WHIP, BIND, MEGA_DRAIN
	db 32, ABSOL,      QUICK_ATTACK, DOUBLE_TEAM, SONICBOOM, SUPERSONIC
	db 35, QUAGSIRE,   TAIL_WHIP, SLAM, AMNESIA, EARTHQUAKE
	db -1 ; end

	; SCHOOLBOY (23)
	db "CHAD@", TRAINERTYPE_NORMAL
	db 30, CAPSAKID
	db 34, BOLTUND
	db -1 ; end

	; SCHOOLBOY (24)
	db "CHAD@", TRAINERTYPE_MOVES
	db 34, CAPSAKID,   PSYCHIC_M, LIGHT_SCREEN, REFLECT, ENCORE
	db 38, BOLTUND,   ZAP_CANNON, THUNDER_WAVE, LOCK_ON, SWIFT
	db -1 ; end

BirdKeeperGroup:
	; BIRD_KEEPER (1)
	db "ROD@", TRAINERTYPE_NORMAL
	db  7, FLETCHLING
	db  7, FLETCHLING
	db -1 ; end

	; BIRD_KEEPER (2)
	db "ABE@", TRAINERTYPE_NORMAL
	db  9, SPEAROW
	db -1 ; end

	; BIRD_KEEPER (3)
	db "ADAM@", TRAINERTYPE_NORMAL
	db 16, HOOTHOOT
	db 18, FLETCHINDR
	db -1 ; end

	; BIRD_KEEPER (4)
	db "DOUG@", TRAINERTYPE_NORMAL
	db 21, FARFETCH_D
	db 20, FARFETCH_D
	db 23, FLETCHINDR
	db 20, SKARMORY
	db 20, SIGILYPH
	db -1 ; end

	; BIRD_KEEPER (5)
	db "DEREK@", TRAINERTYPE_NORMAL
	db 20, TOGEPI
	db 21, CORVISQUIR
	db 22, TOGEPI
	db -1 ; end

	; BIRD_KEEPER (6)
	db "SIMON@", TRAINERTYPE_NORMAL
	db 22, NATU
	db 24, FEAROW
	db 22, FEAROW
	db -1 ; end

	; BIRD_KEEPER (7)
	db "MO@", TRAINERTYPE_NORMAL
	db 38, GOLBAT
	db 38, TALONFLAME
	db -1 ; end

	; BIRD_KEEPER (8)
	db "HANK@", TRAINERTYPE_NORMAL
	db 12, FLETCHLING
	db 34, TALONFLAME
	db -1 ; end

	; BIRD_KEEPER (9)
	db "ROY@", TRAINERTYPE_NORMAL
	db 29, FEAROW
	db 35, FEAROW
	db -1 ; end

	; BIRD_KEEPER (10)
	db "BORIS@", TRAINERTYPE_NORMAL
	db 30, PAWMI
	db 28, PAWMI
	db 32, PAWMO
	db -1 ; end

	; BIRD_KEEPER (11)
	db "BOB@", TRAINERTYPE_NORMAL
	db 34, NOCTOWL
	db -1 ; end

	; BIRD_KEEPER (12)
	db "JOSE@", TRAINERTYPE_NORMAL
	db 36, FARFETCH_D
	db -1 ; end

	; BIRD_KEEPER (13)
	db "STUART@", TRAINERTYPE_NORMAL
	db  8, FLETCHLING
	db  8, FARFETCH_D
	db  10, SPEAROW
	db -1 ; end

	; BIRD_KEEPER (14)
	db "JOSE@", TRAINERTYPE_NORMAL
	db 34, FARFETCH_D
	db -1 ; end

	; BIRD_KEEPER (15)
	db "PERRY@", TRAINERTYPE_NORMAL
	db 34, FARFETCH_D
	db -1 ; end

	; BIRD_KEEPER (16)
	db "BRET@", TRAINERTYPE_NORMAL
	db 32, FLETCHINDR
	db 32, FEAROW
	db -1 ; end

	; BIRD_KEEPER (17)
	db "JOSE@", TRAINERTYPE_MOVES
	db 40, FARFETCH_D, FURY_ATTACK, DETECT, FLY, SLASH
	db -1 ; end

	; BIRD_KEEPER (18)
	db "VANCE@", TRAINERTYPE_NORMAL
	db 32, FLETCHINDR
	db 32, FLETCHINDR
	db -1 ; end

	; BIRD_KEEPER (19)
	db "VANCE@", TRAINERTYPE_MOVES
	db 38, TALONFLAME,    TOXIC, QUICK_ATTACK, WHIRLWIND, FLY
	db 38, TALONFLAME,    SWIFT, DETECT, STEEL_WING, FLY
	db -1 ; end

LassGroup:
	; LASS (1)
	db "CARRIE@", TRAINERTYPE_MOVES
	db 18, MINCCINO,   SCARY_FACE, CHARM, BITE, LICK
	db -1 ; end

	; LASS (2)
	db "BRIDGET@", TRAINERTYPE_NORMAL
	db 15, JIGGLYPUFF
	db 15, JIGGLYPUFF
	db 15, JIGGLYPUFF
	db -1 ; end

	; LASS (3)
	db "ALICE@", TRAINERTYPE_NORMAL
	db 30, GLOOM
	db 34, ZOROARK
	db 30, GLOOM
	db -1 ; end

	; LASS (4)
	db "LOUISE@", TRAINERTYPE_NORMAL
	db 17, ODDISH
	db 20, CUBONE
	db -1 ; end

	; LASS (5)
	db "LAUREN@", TRAINERTYPE_NORMAL
	db 20, JIGGLYPUFF
	db 25, AZUMARILL
	db -1 ; end

	; LASS (6)
	db "LINDA@", TRAINERTYPE_NORMAL
	db 30, BULBASAUR
	db 32, IVYSAUR
	db 34, VENUSAUR
	db -1 ; end

	; LASS (7)
	db "LAURA@", TRAINERTYPE_NORMAL
	db 28, GLOOM
	db 31, FLETCHINDR
	db 31, BELLOSSOM
	db -1 ; end

	; LASS (8)
	db "SHANNON@", TRAINERTYPE_NORMAL
	db 29, PARAS
	db 29, PARAS
	db 32, PARASECT
	db -1 ; end

	; LASS (9)
	db "MICHELLE@", TRAINERTYPE_NORMAL
	db 32, DOLLIV
	db 33, SMOLIV
	db 34, ARBOLIVA
	db -1 ; end

	; LASS (10)
	db "STEPHANIE@", TRAINERTYPE_MOVES
	db 23, MAREEP,     TACKLE, THUNDERSHOCK, THUNDER_WAVE, COTTON_SPORE
	db 23, PSYDUCK,    SCRATCH, DISABLE, CONFUSION, SCREECH
	db -1 ; end

	; LASS (11)
	db "ELLEN@", TRAINERTYPE_NORMAL
	db 30, WIGGLYTUFF
	db 34, CINCCINO
	db -1 ; end

	; LASS (12)
	db "CLAIRE@", TRAINERTYPE_NORMAL
	db 11, SUNKERN
	db 11, WOOPER
	db -1 ; end

	; LASS (13)
	db "CONNIE@", TRAINERTYPE_NORMAL
	db 21, MARILL
	db -1 ; end

	; LASS (14)
	db "DANA@", TRAINERTYPE_MOVES
	db 21, FLAAFFY,    TACKLE, GROWL, THUNDERSHOCK, THUNDER_WAVE
	db 21, PSYDUCK,    SCRATCH, TAIL_WHIP, DISABLE, CONFUSION
	db -1 ; end

	; LASS (15)
	db "DANA@", TRAINERTYPE_MOVES
	db 29, PSYDUCK,    SCRATCH, DISABLE, CONFUSION, SCREECH
	db 29, AMPHAROS,   TACKLE, THUNDERSHOCK, THUNDER_WAVE, COTTON_SPORE
	db -1 ; end

	; LASS (16)
	db "DANA@", TRAINERTYPE_MOVES
	db 32, PSYDUCK,    SCRATCH, DISABLE, CONFUSION, SCREECH
	db 32, AMPHAROS,   TACKLE, THUNDERPUNCH, THUNDER_WAVE, COTTON_SPORE
	db -1 ; end

	; LASS (17)
	db "DANA@", TRAINERTYPE_MOVES
	db 36, AMPHAROS,   SWIFT, THUNDERPUNCH, THUNDER_WAVE, COTTON_SPORE
	db 36, GOLDUCK,    DISABLE, SURF, PSYCHIC_M, SCREECH
	db -1 ; end

JanineGroup:
	; JANINE (1)
	db "JANINE@", TRAINERTYPE_MOVES
	db 36, CROBAT,     SCREECH, SUPERSONIC, CONFUSE_RAY, WING_ATTACK
	db 36, WEEZING,    SMOG, SLUDGE_BOMB, TOXIC, EXPLOSION
	db 36, WEEZING,    SMOG, SLUDGE_BOMB, TOXIC, EXPLOSION
	db 33, SWALOT,    SCARY_FACE, GIGA_DRAIN, STRING_SHOT, NIGHT_SHADE
	db 39, FROSMOTH,   FORESIGHT, DOUBLE_TEAM, GUST, PSYCHIC_M
	db -1 ; end

CooltrainerMGroup:
	; COOLTRAINERM (1)
	db "GARETH@", TRAINERTYPE_MOVES
	db 26, CHARMELEON, BELLY_DRUM, SCRATCH, EMBER, METAL_CLAW
	db 26, LEDIAN,     LIGHT_SCREEN, SUPERSONIC, COMET_PUNCH, REFLECT
	db 26, HOUNDOOM,   FIRE_BLAST, LEER, SMOG, BITE
	db -1 ; end

	; COOLTRAINERM (2)
	db "BLAIR@", TRAINERTYPE_DVS | TRAINERTYPE_MOVES
	db 33, GOLDUCK
		db $d8, $c8
		db DYNAMICPUNCH, SCRATCH, CONFUSION, SCREECH
	db 33, GULPIN
		db $d8, $c8
		db SLUDGE_BOMB, POUND, AMNESIA, TOXIC
	db 33, GYARADOS
		db $c9, $c8
		db HIDDEN_POWER, THRASH, BITE, DRAGON_RAGE
	db -1 ; end

	; COOLTRAINERM (3)
	db "BREND@", TRAINERTYPE_DVS | TRAINERTYPE_MOVES
	db 37, UNOWN
		db $cc, $c8
		db HIDDEN_POWER, NO_MOVE, NO_MOVE, NO_MOVE
	db 37, KADABRA
		db $d8, $c8
		db RECOVER, FIRE_PUNCH, PSYBEAM, FUTURE_SIGHT
	db 37, WOBBUFFET
		db $d8, $c8
		db COUNTER, MIRROR_COAT, SAFEGUARD, DESTINY_BOND
	db -1 ; end

	; COOLTRAINERM (4)
	db "TONY@", TRAINERTYPE_MOVES
	db 37, HYPNO,      THUNDER_WAVE, HYPNOSIS, CONFUSION, HEADBUTT
	db 39, SLOWKING,   SURF, CONFUSION, DISABLE, HEADBUTT
	db -1 ; end

	; COOLTRAINERM (5)
	db "ANTONY@", TRAINERTYPE_MOVES
	db 35, SIGILYPH,   ICE_BEAM, HYPNOSIS, PSYBEAM, AIR_CUTTER
	db 37, SLOWBRO,    PSYCHIC_M, WATER_GUN, HEADBUTT, DEFENSE_CURL
	db -1 ; end

	; COOLTRAINERM (6)
	db "GAVEN@", TRAINERTYPE_MOVES
	db 35, VICTREEBEL, WRAP, TOXIC, ACID, RAZOR_LEAF
	db 35, KINGLER,    BUBBLEBEAM, STOMP, GUILLOTINE, PROTECT
	db 35, FLAREON,    SAND_ATTACK, QUICK_ATTACK, BITE, FIRE_SPIN
	db -1 ; end

	; COOLTRAINERM (7)
	db "GAVEN@", TRAINERTYPE_ITEM_MOVES
	db 39, VICTREEBEL, NO_ITEM,      GIGA_DRAIN, TOXIC, SLUDGE_BOMB, RAZOR_LEAF
	db 39, KINGLER,    KINGS_ROCK,   SURF, STOMP, GUILLOTINE, BLIZZARD
	db 39, FLAREON,    NO_ITEM,      FLAMETHROWER, QUICK_ATTACK, BITE, FIRE_SPIN
	db -1 ; end

	; COOLTRAINERM (8)
	db "RYAN@", TRAINERTYPE_MOVES
	db 25, TALONFLAME,    SAND_ATTACK, QUICK_ATTACK, WHIRLWIND, WING_ATTACK
	db 27, WHIRLIPEDE, THUNDERPUNCH, LIGHT_SCREEN, SWIFT, SCREECH
	db -1 ; end

	; COOLTRAINERM (9)
	db "JAKE@", TRAINERTYPE_MOVES
	db 33, PARASECT,   LEECH_LIFE, SPORE, SLASH, SWORDS_DANCE
	db 35, GOLDUCK,    CONFUSION, SCREECH, PSYCH_UP, FURY_SWIPES
	db -1 ; end

	; COOLTRAINERM (10)
	db "GAVEN@", TRAINERTYPE_MOVES
	db 32, VICTREEBEL, WRAP, TOXIC, ACID, RAZOR_LEAF
	db 32, KINGLER,    BUBBLEBEAM, STOMP, GUILLOTINE, PROTECT
	db 32, FLAREON,    SAND_ATTACK, QUICK_ATTACK, BITE, FIRE_SPIN
	db -1 ; end

	; COOLTRAINERM (11)
	db "BLAKE@", TRAINERTYPE_MOVES
	db 33, BOLTUND,   THUNDERBOLT, SUPERSONIC, SWIFT, SCREECH
	db 31, QUAGSIRE,   WATER_GUN, SLAM, AMNESIA, EARTHQUAKE
	db 31, RIOLU,  LEECH_SEED, CONFUSION, SLEEP_POWDER, SOLARBEAM
	db -1 ; end

	; COOLTRAINERM (12)
	db "BRIAN@", TRAINERTYPE_MOVES
	db 35, SANDSLASH,  SAND_ATTACK, POISON_STING, SLASH, SWIFT
	db -1 ; end

	; COOLTRAINERM (13)
	db "IAIN@", TRAINERTYPE_NORMAL
	db 13, PHANPY
	db 18, RHYHORN
	db -1 ; end

	; COOLTRAINERM (14)
	db "PAT@", TRAINERTYPE_NORMAL
	db 24, LUCARIO
	db 24, SKARMORY
	db 26, LUCARIO
	db -1 ; end

	; COOLTRAINERM (15)
	db "TYLER@", TRAINERTYPE_NORMAL
	db 10, BULBASAUR
	db 10, CHARMANDER
	db 10, SQUIRTLE
	db -1 ; end

	; COOLTRAINERM (16)
	db "SEAN@", TRAINERTYPE_NORMAL
	db 35, FLAREON
	db 35, TANGELA
	db 35, TAUROS
	db -1 ; end

	; COOLTRAINERM (17)
	db "KEVIN@", TRAINERTYPE_NORMAL
	db 38, RHYHORN
	db 35, CHARMELEON
	db 35, WARTORTLE
	db -1 ; end

	; COOLTRAINERM (18)
	db "STEVE@", TRAINERTYPE_NORMAL
	db 14, BULBASAUR
	db 14, CHARMANDER
	db 14, SQUIRTLE
	db -1 ; end

	; COOLTRAINERM (19)
	db "GREGOR@", TRAINERTYPE_MOVES
	db 35, KINGLER,    BODY_SLAM, LEER, MUD_SHOT, METAL_CLAW
	db 40, HOUNDOOM,   CRUNCH, PURSUIT, EMBER, SMOG
	db -1 ; end

	; COOLTRAINERM (20)
	db "DARIN@", TRAINERTYPE_MOVES
	db 37, DRAGONAIR,  WRAP, SURF, DRAGON_RAGE, SLAM
	db -1 ; end

CooltrainerFGroup:
	; COOLTRAINERF (1)
	db "KELLY@", TRAINERTYPE_MOVES
	db 26, LUCARIO,    SWORDS_DANCE, QUICK_ATTACK, REVERSAL, METAL_CLAW
	db 23, SHUCKLE,    DEFENSE_CURL, WRAP, ENCORE, SAFEGUARD
	db 23, STANTLER,   TAKE_DOWN, LEER, HYPNOSIS, STOMP
	db 23, AZUMARILL,  SURF, DEFENSE_CURL, TAIL_WHIP, ROLLOUT
	db -1 ; end

	; COOLTRAINERF (2)
	db "ALISON@", TRAINERTYPE_MOVES
	db 34, ALAKAZAM,   PSYCHIC_M, KINESIS, RECOVER, FUTURE_SIGHT
	db 34, ARCANINE,   FIRE_BLAST, BITE, TAKE_DOWN, FLAME_WHEEL
	db -1 ; end

	; COOLTRAINERF (3)
	db "SUSAN@", TRAINERTYPE_MOVES
	db 35, GIRAFARIG,  EARTHQUAKE, CONFUSION, STOMP, AGILITY
	db 40, XATU,       PSYCHIC_M, PECK, NIGHT_SHADE, FUTURE_SIGHT
	db -1 ; end

	; COOLTRAINERF (4)
	db "ANGELA@", TRAINERTYPE_MOVES
	db 37, STARMIE,    SURF, RECOVER, SWIFT, CONFUSE_RAY
	db 39, ESPEON,     PSYCHIC_M, CHARM, SAND_ATTACK, SWIFT
	db -1 ; end

	; COOLTRAINERF (5)
	db "DONNA@", TRAINERTYPE_MOVES
	db 28, LANTURN,    SURF, THUNDER_WAVE, FLAIL, SPARK
	db 30, SHUCKLE,    DEFENSE_CURL, TOXIC, WRAP, BIDE
	db -1 ; end

	; COOLTRAINERF (6)
	db "LYNSEY@", TRAINERTYPE_MOVES
	db 25, SEEL,       SURF, HEADBUTT, AURORA_BEAM, REST
	db 26, GREAVARD,   REST, TACKLE, LICK, BITE
	db -1 ; end

	; COOLTRAINERF (7)
	db "KELLY@", TRAINERTYPE_NORMAL
	db 27, MARILL
	db 24, WARTORTLE
	db 24, WARTORTLE
	db -1 ; end

	; COOLTRAINERF (8)
	db "JOYCE@", TRAINERTYPE_MOVES
	db 36, LUXIO,    QUICK_ATTACK, DOUBLE_TEAM, THUNDERBOLT, THUNDER
	db 32, BLASTOISE,  BITE, CURSE, SURF, RAIN_DANCE
	db -1 ; end

	; COOLTRAINERF (9)
	db "BETH@", TRAINERTYPE_MOVES
	db 36, RAPIDASH,   STOMP, FIRE_SPIN, FURY_ATTACK, AGILITY
	db -1 ; end

	; COOLTRAINERF (10)
	db "REENA@", TRAINERTYPE_NORMAL
	db 31, STARMIE
	db 33, NIDOQUEEN
	db 31, STARMIE
	db -1 ; end

	; COOLTRAINERF (11)
	db "MEGAN@", TRAINERTYPE_MOVES
	db 32, BULBASAUR,  GROWL, LEECH_SEED, POISONPOWDER, RAZOR_LEAF
	db 32, IVYSAUR,    GROWL, LEECH_SEED, POISONPOWDER, RAZOR_LEAF
	db 32, VENUSAUR,   BODY_SLAM, SLEEP_POWDER, RAZOR_LEAF, SWEET_SCENT
	db -1 ; end

	; COOLTRAINERF (12)
	db "BETH@", TRAINERTYPE_MOVES
	db 39, RAPIDASH,   STOMP, FIRE_SPIN, FURY_ATTACK, AGILITY
	db -1 ; end

	; COOLTRAINERF (13)
	db "CAROL@", TRAINERTYPE_NORMAL
	db 35, ZEBSTRIKA
	db 35, STARMIE
	db 35, NINETALES
	db -1 ; end

	; COOLTRAINERF (14)
	db "QUINN@", TRAINERTYPE_NORMAL
	db 38, IVYSAUR
	db 38, STARMIE
	db -1 ; end

	; COOLTRAINERF (15)
	db "RACHEL@", TRAINERTYPE_MOVES
	db 23, DURANT,     VICEGRIP, SAND_ATTACK, FURY_CUTTER, BITE
	db 28, KABUTO,     ANCIENTPOWER, SCRATCH, ABSORB, SAND_ATTACK
	db -1 ; end

	; COOLTRAINERF (16)
	db "JULIE@", TRAINERTYPE_MOVES
	db 38, SIGILYPH,   PSYCHIC_M, HYPNOSIS, AIR_CUTTER, LIGHT_SCREEN
	db 38, STEELIX,    EARTHQUAKE, TACKLE, ROCK_THROW, DRAGONBREATH
	db -1 ; end

	; COOLTRAINERF (17)
	db "LEANNE@", TRAINERTYPE_MOVES
	db 26, SMEARGLE,   SPORE, PAIN_SPLIT, DRAGON_RAGE, NO_MOVE
	db 28, LUXIO,      THUNDERBOLT, BITE, ROAR, SWAGGER
	db -1 ; end

	; COOLTRAINERF (18)
	db "BETH@", TRAINERTYPE_ITEM_MOVES
	db 43, RAPIDASH,   FOCUS_BAND,   STOMP, FIRE_SPIN, FURY_ATTACK, FIRE_BLAST
	db -1 ; end

	; COOLTRAINERF (19)
	db "REENA@", TRAINERTYPE_NORMAL
	db 34, STARMIE
	db 36, NIDOQUEEN
	db 34, STARMIE
	db -1 ; end

	; COOLTRAINERF (20)
	db "REENA@", TRAINERTYPE_ITEM_MOVES
	db 38, STARMIE,    NO_ITEM,      DOUBLE_TEAM, PSYCHIC_M, WATERFALL, CONFUSE_RAY
	db 40, NIDOQUEEN,  PINK_BOW,     EARTHQUAKE, DOUBLE_KICK, TOXIC, BODY_SLAM
	db 38, STARMIE,    NO_ITEM,      BLIZZARD, PSYCHIC_M, WATERFALL, RECOVER
	db -1 ; end

	; COOLTRAINERF (21)
	db "CARA@", TRAINERTYPE_MOVES
	db 33, HORSEA,     SMOKESCREEN, LEER, WHIRLPOOL, TWISTER
	db 33, HORSEA,     SMOKESCREEN, LEER, WHIRLPOOL, TWISTER
	db 35, SEADRA,     SWIFT, LEER, WATERFALL, TWISTER
	db -1 ; end

BeautyGroup:
	; BEAUTY (1)
	db "VICTORIA@", TRAINERTYPE_NORMAL
	db  9, SENTRET
	db 13, SENTRET
	db 17, SENTRET
	db -1 ; end

	; BEAUTY (2)
	db "SAMANTHA@", TRAINERTYPE_MOVES
	db 16, MEOWTH,     SCRATCH, GROWL, BITE, PAY_DAY
	db 16, MEOWTH,     SCRATCH, GROWL, BITE, SLASH
	db -1 ; end

	; BEAUTY (3)
	db "NICOLA@", TRAINERTYPE_NORMAL
	db 13, SMOLIV
	db 13, PARAS
	db -1 ; end

	; BEAUTY (4)
	db "JENNIFER@", TRAINERTYPE_NORMAL
	db 11, BELLSPROUT
	db 11, CAPSAKID
	db 13, SPRIGATITO
	db -1 ; end

	; BEAUTY (5)
	db "BRENDA@", TRAINERTYPE_NORMAL
	db 16, FURRET
	db -1 ; end

	; BEAUTY (6)
	db "CASSIE@", TRAINERTYPE_NORMAL
	db 28, VILEPLUME
	db 34, VIVILLON
	db -1 ; end

	; BEAUTY (7)
	db "CAROLINE@", TRAINERTYPE_NORMAL
	db 30, MARILL
	db 32, SEEL
	db 30, MARILL
	db -1 ; end

	; BEAUTY (8)
	db "CARLENE@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db -1 ; end

	; BEAUTY (9)
	db "JESSICA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db -1 ; end

	; BEAUTY (10)
	db "RACHAEL@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db -1 ; end

	; BEAUTY (11)
	db "ANGELICA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db -1 ; end

	; BEAUTY (12)
	db "KENDRA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db -1 ; end

	; BEAUTY (13)
	db "VERONICA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db -1 ; end

	; BEAUTY (14)
	db "JULIA@", TRAINERTYPE_NORMAL
	db 32, PARAS
	db 32, RIOLU
	db 35, PARASECT
	db -1 ; end

	; BEAUTY (15)
	db "THERESA@", TRAINERTYPE_NORMAL
	db 15, SENTRET
	db -1 ; end

	; BEAUTY (16)
	db "FIONA@", TRAINERTYPE_MOVES
	db 22, JIGGLYPUFF, SING, DEFENSE_CURL, POUND, ROLLOUT
	db 22, SMOLIV,     GROWTH, RAZOR_LEAF, FLAIL, MEGA_DRAIN
	db -1 ; end

	; BEAUTY (17)
	db "STACEY@", TRAINERTYPE_NORMAL
	db 19, HORSEA
	db 24, MEOWTH
	db -1 ; end

PokemaniacGroup:
	; POKEMANIAC (1)
	db "KEVIN@", TRAINERTYPE_NORMAL
	db  8, NIDORAN_F
	db 12, NIDORAN_M
	db -1 ; end

	; POKEMANIAC (2)
	db "CAMERON@", TRAINERTYPE_MOVES
	db 24, WARTORTLE,  TACKLE, DEFENSE_CURL, WATER_GUN, BITE
	db 24, CHARMELEON, BITE, EMBER, METAL_CLAW, OUTRAGE
	db -1 ; end

	; POKEMANIAC (3)
	db "CHARLES@", TRAINERTYPE_MOVES
	db 23, CHARMELEON, BITE, EMBER, ROCK_SLIDE, OUTRAGE
	db 26, KANGASKHAN, COMET_PUNCH, LEER, BITE, MEGA_PUNCH
	db -1 ; end

	; POKEMANIAC (4)
	db "AARON@", TRAINERTYPE_MOVES
	db 29, NIDOKING,   ROCK_TOMB, DOUBLE_KICK, TAKE_DOWN, CONFUSION
	db 29, MAROWAK,    HEADBUTT, LEER, FOCUS_ENERGY, BONEMERANG
	db -1 ; end

	; POKEMANIAC (5)
	db "RAYMOND@", TRAINERTYPE_MOVES
	db 27, NIDOQUEEN,  DOUBLE_KICK, ROCK_TOMB, BITE, BODY_SLAM
	db 31, NIDOQUEEN,  ICE_PUNCH, ROCK_TOMB, BITE, BODY_SLAM
	db -1 ; end

	; POKEMANIAC (6)
	db "KIERAN@", TRAINERTYPE_MOVES
	db 27, TOTODILE,    SCRATCH, LEER, WATER_GUN, BITE
	db 31, LICKITUNG,   LICK, SUPERSONIC, STOMP, WRAP
	db -1 ; end

	; POKEMANIAC (7)
	db "RORY@", TRAINERTYPE_MOVES
	db 27, SLOWPOKE,   CURSE, TACKLE, WATER_GUN, CONFUSION
	db 31, WARTORTLE,  ICE_PUNCH, HEADBUTT, WATER_GUN, BITE
	db -1 ; end

	; POKEMANIAC (8)
	db "ETHAN@", TRAINERTYPE_NORMAL
	db 31, RHYHORN
	db 31, RHYDON
	db -1 ; end

	; POKEMANIAC (9)
	db "BRENT@", TRAINERTYPE_NORMAL
	db 25, KANGASKHAN
	db -1 ; end

	; POKEMANIAC (10)
	db "BRENT@", TRAINERTYPE_MOVES
	db 36, PORYGON,    RECOVER, PSYCHIC_M, CONVERSION2, TRI_ATTACK
	db -1 ; end

	; POKEMANIAC (11)
	db "RICHARD@", TRAINERTYPE_NORMAL
	db 12, CHARMANDER
	db 17, IVYSAUR
	db -1 ; end

	; POKEMANIAC (12)
	db "COLIN@", TRAINERTYPE_MOVES
	db 16, CHARMELEON, SCRATCH, BITE, EMBER, METAL_CLAW
	db 16, NIDORINO,   CONFUSION, TACKLE, HORN_ATTACK, DOUBLE_KICK
	db -1 ; end

	; POKEMANIAC (13)
	db "DARYL@", TRAINERTYPE_MOVES
	db 35, AMPHAROS,   TACKLE, GROWL, THUNDER_WAVE, THUNDERPUNCH
	db 40, LAPRAS,     WATER_GUN, BODY_SLAM, ICE_BEAM, THUNDERBOLT
	db -1 ; end

	; POKEMANIAC (14)
	db "BRENT@", TRAINERTYPE_MOVES
	db 41, CHANSEY,    ROLLOUT, ATTRACT, EGG_BOMB, SOFTBOILED
	db -1 ; end

	; POKEMANIAC (15)
	db "LEWIS@", TRAINERTYPE_MOVES
	db 29, LAPRAS,     WATER_GUN, SING, BODY_SLAM, CONFUSE_RAY
	db 29, WARTORTLE,  TACKLE, HEADBUTT, WATER_GUN, BITE
	db -1 ; end

GruntMGroup:
	; GRUNTM (1)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 9, RATTATA
	db 14, KOFFING
	db -1 ; end

	; GRUNTM (2)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 9, RATTATA
	db 9, SANDSHREW
	db 9, ZUBAT
	db -1 ; end

	; GRUNTM (3)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, SANDSLASH
	db 30, RATICATE
	db -1 ; end

	; GRUNTM (4)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, GOLBAT
	db 30, GROWLITHE
	db 31, GRIMER
	db -1 ; end

	; GRUNTM (5)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, GLOOM
	db 30, HOUNDOOM
	db 30, SWALOT
	db 30, SNEASEL
	db 30, RATICATE
	db -1 ; end

	; GRUNTM (6)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 32, HAUNTER
	db 32, GOLBAT
	db -1 ; end

	; GRUNTM (7)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, KOFFING
	db 30, GRIMER
	db 30, GOLBAT
	db 30, RATICATE
	db -1 ; end

	; GRUNTM (8)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, PERRSERKER
	db 32, KOFFING
	db -1 ; end

	; GRUNTM (9)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, RATICATE
	db 32, KOFFING
	db -1 ; end

	; GRUNTM (10)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, HAUNTER
	db 30, GOLBAT
	db 30, GRIMER
	db -1 ; end

	; GRUNTM (11)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, GRIMER
	db 30, KOFFING
	db 31, RATICATE
	db -1 ; end

	; GRUNTM (12)
	db "FRASER@", TRAINERTYPE_NORMAL
	db 17, HOUNDOUR
	db 17, ZORUA
	db 17, SNEASEL
	db 17, ABSOL
	db 17, SPIRITOMB
	db -1 ; end

	; GRUNTM (13)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, GROWLITHE
	db 33, RATICATE
	db -1 ; end

	; GRUNTM (14)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, RATICATE
	db 30, GOLBAT
	db -1 ; end

	; GRUNTM (15)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 32, GRIMER
	db 30, KOFFING
	db -1 ; end

	; GRUNTM (16)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 27, MAGMAR
	db 27, SNEASEL
	db 27, LARVITAR
	db 27, RATICATE
	db -1 ; end

	; GRUNTM (17)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 27, MUDBRAY
	db 27, GOLBAT
	db -1 ; end

	; GRUNTM (18)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 27, DURANT
	db 27, GOLBAT
	db 27, RATICATE
	db -1 ; end

	; GRUNTM (19)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 27, PARASECT
	db 27, SNEASEL
	db -1 ; end

	; GRUNTM (20)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 27, HYPNO
	db 27, GOLBAT
	db -1 ; end

	; GRUNTM (21)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 27, GOLBAT
	db 27, GRIMER
	db 27, RATICATE
	db -1 ; end

	; GRUNTM (22)
	db "SHAUN@", TRAINERTYPE_NORMAL
	db 17, SNEASEL
	db 22, ABSOL
	db -1 ; end

	; GRUNTM (23)
	db "EXECUTIVE@", TRAINERTYPE_NORMAL
	db 30, KOFFING
	db -1 ; end

	; GRUNTM (24)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 31, HOUNDOOM
	db 31, KOFFING
	db -1 ; end

	; GRUNTM (25)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, KOFFING
	db 30, GRIMER
	db -1 ; end

	; GRUNTM (26)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 15, RATTATA
	db 15, RATTATA
	db -1 ; end

	; GRUNTM (27)
	db "EXECUTIVE@", TRAINERTYPE_NORMAL
	db 22, ZUBAT
	db -1 ; end

	; GRUNTM (28)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 27, HYPNO
	db 27, RATICATE
	db -1 ; end

	; GRUNTM (29)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 9, RATTATA
	db 9, RATTATA
	db -1 ; end

	; GRUNTM (30)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 25, GOLBAT
	db 25, GOLBAT
	db 30, ZOROARK
	db -1 ; end

	; GRUNTM (31)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, GOLBAT
	db -1 ; end

GentlemanGroup:
	; GENTLEMAN (1)
	db "Nicholas@", TRAINERTYPE_NORMAL
	db 22, NIDORINA
	db 22, GROWLITHE
	db -1 ; end

	; GENTLEMAN (2)
	db "EDWARD@", TRAINERTYPE_NORMAL
	db 33, PERRSERKER
	db -1 ; end

	; GENTLEMAN (3)
	db "GREGORY@", TRAINERTYPE_NORMAL
	db 37, LUXIO
	db 33, FLAAFFY
	db -1 ; end

	; GENTLEMAN (4)
	db "GERARD@", TRAINERTYPE_NORMAL
	db 31, BOLTUND
	db 36, ZEBSTRIKA
	db -1 ; end

	; GENTLEMAN (5)
	db "STEWART@", TRAINERTYPE_NORMAL
	db 20, NIDORINA
	db 24, NOCTOWL
	db -1 ; end

SkierGroup:
	; SKIER (1)
	db "CHARLENE@", TRAINERTYPE_NORMAL
	db 25, SWINUB
	db 25, DELIBIRD
	db -1 ; end

	; SKIER (2)
	db "CLARISSA@", TRAINERTYPE_NORMAL
	db 28, DEWGONG
	db -1 ; end

TeacherGroup:
	; TEACHER (1)
	db "COLETTE@", TRAINERTYPE_NORMAL
	db 36, CLEFAIRY
	db -1 ; end

	; TEACHER (2)
	db "HILLARY@", TRAINERTYPE_NORMAL
	db 32, AIPOM
	db 36, CUBONE
	db -1 ; end

	; TEACHER (3)
	db "SHIRLEY@", TRAINERTYPE_NORMAL
	db 35, JIGGLYPUFF
	db -1 ; end

SabrinaGroup:
	; SABRINA (1)
	db "SABRINA@", TRAINERTYPE_MOVES
	db 46, ESPEON,     SAND_ATTACK, QUICK_ATTACK, SWIFT, PSYCHIC_M
	db 46, CAPSAKID,   BARRIER, REFLECT, BATON_PASS, PSYCHIC_M
	db 48, ALAKAZAM,   RECOVER, FUTURE_SIGHT, PSYCHIC_M, REFLECT
	db -1 ; end

BugCatcherGroup:
	; BUG_CATCHER (1)
	db "ANDREW@", TRAINERTYPE_NORMAL
	db  5, SCATTERBUG
	db  5, SCATTERBUG
	db -1 ; end

	; BUG_CATCHER (2)
	db "ROB@", TRAINERTYPE_NORMAL
	db 32, PAWMOT
	db 32, VIVILLON
	db -1 ; end

	; BUG_CATCHER (3)
	db "ED@", TRAINERTYPE_NORMAL
	db 30, PAWMOT
	db 30, PAWMOT
	db 30, PAWMOT
	db -1 ; end

	; BUG_CATCHER (4)
	db "JAMES@", TRAINERTYPE_NORMAL
	db  5, SNOM
	db  5, SNOM
	db  6, SCATTERBUG
	db  5, SCATTERBUG
	db -1 ; end

	; BUG_CATCHER (5)
	db "LEE@", TRAINERTYPE_NORMAL
	db 15, ONIX
	db 15, MUDBRAY
	db 15, GLIGAR
	db -1 ; end

	; BUG_CATCHER (6)
	db "PHILIP@", TRAINERTYPE_MOVES
	db 26, DURANT,     VICEGRIP, FURY_CUTTER, BITE, AGILITY
	db 26, LUCARIO,    QUICK_ATTACK, REVERSAL, METAL_CLAW, CRUNCH
	db -1 ; end

	; BUG_CATCHER (7)
	db "JOSH@", TRAINERTYPE_NORMAL
	db 13, PARAS
	db -1 ; end

	; BUG_CATCHER (8)
	db "BARRY@", TRAINERTYPE_NORMAL
	db 15, VIVILLON
	db 19, LEDIAN
	db -1 ; end

	; BUG_CATCHER (9)
	db "KEN@", TRAINERTYPE_NORMAL
	db 30, SWALOT
	db 32, DURANT
	db -1 ; end

	; BUG_CATCHER (10)
	db "JAMES@", TRAINERTYPE_NORMAL
	db 16, SNOM
	db 16, SNOM
	db 17, VIVILLON
	db 16, SPEWPA
	db -1 ; end

	; BUG_CATCHER (11)
	db "JAMES@", TRAINERTYPE_NORMAL
	db 14, VIVILLON
	db 14, VIVILLON
	db 15, PAWMOT
	db 14, VIVILLON
	db -1 ; end

	; BUG_CATCHER (12)
	db "DOUG@", TRAINERTYPE_NORMAL
	db 34, SWALOT
	db -1 ; end

	; BUG_CATCHER (13)
	db "ARNIE@", TRAINERTYPE_NORMAL
	db 19, SNOM
	db -1 ; end

	; BUG_CATCHER (14)
	db "ARNIE@", TRAINERTYPE_MOVES
	db 28, FROSMOTH,   DISABLE, SUPERSONIC, CONFUSION, LEECH_LIFE
	db -1 ; end

	; BUG_CATCHER (15)
	db "JAMES@", TRAINERTYPE_MOVES
	db 24, VIVILLON, CONFUSION, POISONPOWDER, SUPERSONIC, WHIRLWIND
	db 24, VIVILLON, CONFUSION, STUN_SPORE, SUPERSONIC, WHIRLWIND
	db 25, PAWMOT,   FURY_ATTACK, FOCUS_ENERGY, TACKLE, RAGE
	db 24, VIVILLON, CONFUSION, SLEEP_POWDER, SUPERSONIC, WHIRLWIND
	db -1 ; end

	; BUG_CATCHER (16)
	db "JAMES@", TRAINERTYPE_MOVES
	db 30, VIVILLON, CONFUSION, POISONPOWDER, SUPERSONIC, GUST
	db 30, VIVILLON, CONFUSION, STUN_SPORE, SUPERSONIC, GUST
	db 32, PAWMOT,   FURY_ATTACK, PURSUIT, TACKLE, DOUBLE_TEAM
	db 34, VIVILLON, PSYBEAM, SLEEP_POWDER, GUST, WHIRLWIND
	db -1 ; end

	; BUG_CATCHER (17)
	db "ARNIE@", TRAINERTYPE_MOVES
	db 36, FROSMOTH,   GUST, SUPERSONIC, PSYBEAM, LEECH_LIFE
	db -1 ; end

	; BUG_CATCHER (18)
	db "ARNIE@", TRAINERTYPE_MOVES
	db 40, FROSMOTH,   GUST, SUPERSONIC, PSYCHIC_M, TOXIC
	db -1 ; end

	; BUG_CATCHER (19)
	db "MARTIN@", TRAINERTYPE_NORMAL
	db 13, SPEWPA
	db 15, VENIPEDE
	db -1 ; end

FisherGroup:
	; FISHER (1)
	db "MARK@", TRAINERTYPE_NORMAL
	db  7, SHELLDER
	db  7, TENTACOOL
	db 15, MAGIKARP
	db  7, CHINCHOU
	db -1 ; end

	; FISHER (2)
	db "PAUL@", TRAINERTYPE_NORMAL
	db 15, MAGIKARP
	db 15, MAGIKARP
	db -1 ; end

	; FISHER (3)
	db "ARNOLD@", TRAINERTYPE_NORMAL
	db 34, TENTACRUEL
	db -1 ; end

	; FISHER (4)
	db "KYLE@", TRAINERTYPE_NORMAL
	db 28, TOXAPEX
	db 31, LYCANROC
	db 31, TOXAPEX
	db -1 ; end

	; FISHER (5)
	db "SCOTT@", TRAINERTYPE_NORMAL
	db  10, STARYU
	db  10, HORSEA
	db -1 ; end

	; FISHER (6)
	db "ALASDAIR@", TRAINERTYPE_NORMAL
	db 27, KRABBY
	db 27, LANTURN
	db 27, GYARADOS
	db 27, GYARADOS
	db -1 ; end

	; FISHER (7)
	db "ALASTAIR@", TRAINERTYPE_NORMAL
	db 26, STARYU
	db 31, QWILFISH
	db -1 ; end

	; FISHER (8)
	db "KRISTOPHER@", TRAINERTYPE_NORMAL
	db 31, QWILFISH
	db 36, GYARADOS
	db -1 ; end

	; FISHER (9)
	db "NIALL@", TRAINERTYPE_NORMAL
	db 31, PSYDUCK
	db 31, SLOWPOKE
	db 31, STARYU
	db 31, GYARADOS
	db -1 ; end

	; FISHER (10)
	db "ANGUS@", TRAINERTYPE_NORMAL
	db 36, QWILFISH
	db 36, LANTURN
	db 38, GOLDUCK
	db -1 ; end

	; FISHER (11)
	db "BEN@", TRAINERTYPE_MOVES
	db 38, GYARADOS,   THRASH, BITE, DRAGON_RAGE, TWISTER
	db 38, OCTILLERY,  PSYBEAM, AURORA_BEAM, BUBBLEBEAM, FOCUS_ENERGY
	db -1 ; end

	; FISHER (12)
	db "JONAH@", TRAINERTYPE_NORMAL
	db 25, SHELLDER
	db 29, OCTILLERY
	db 25, REMORAID
	db 29, CLOYSTER
	db -1 ; end

	; FISHER (13)
	db "MARTIN@", TRAINERTYPE_NORMAL
	db 32, REMORAID
	db 32, REMORAID
	db -1 ; end

	; FISHER (14)
	db "STEPHEN@", TRAINERTYPE_NORMAL
	db 25, MAGIKARP
	db 25, MAGIKARP
	db 31, QWILFISH
	db 31, TENTACRUEL
	db -1 ; end

	; FISHER (15)
	db "BARNEY@", TRAINERTYPE_NORMAL
	db 30, GYARADOS
	db 30, GYARADOS
	db 30, GYARADOS
	db -1 ; end

	; FISHER (16)
	db "PAUL@", TRAINERTYPE_NORMAL
	db 19, MAGIKARP
	db 19, MAGIKARP
	db -1 ; end

	; FISHER (17)
	db "RALPH@", TRAINERTYPE_NORMAL
	db 17, QWILFISH
	db 19, MAREANIE
	db -1 ; end

	; FISHER (18)
	db "TULLY@", TRAINERTYPE_NORMAL
	db 23, QWILFISH
	db -1 ; end

	; FISHER (19)
	db "TULLY@", TRAINERTYPE_NORMAL
	db 32, MAREANIE
	db 32, MAREANIE
	db 32, QWILFISH
	db -1 ; end

	; FISHER (20)
	db "WILTON@", TRAINERTYPE_NORMAL
	db 29, MAREANIE
	db 29, MAREANIE
	db 32, TOXAPEX
	db -1 ; end

	; FISHER (21)
	db "SCOTT@", TRAINERTYPE_NORMAL
	db 30, QWILFISH
	db 30, QWILFISH
	db 34, TOXAPEX
	db -1 ; end

	; FISHER (22)
	db "WILTON@", TRAINERTYPE_MOVES
	db 34, TOXAPEX,    SUPERSONIC, WATERFALL, FLAIL, FURY_ATTACK
	db 34, TOXAPEX,    SUPERSONIC, WATERFALL, FLAIL, FURY_ATTACK
	db 38, REMORAID,   PSYBEAM, AURORA_BEAM, BUBBLEBEAM, HYPER_BEAM
	db -1 ; end

	; FISHER (23)
	db "RALPH@", TRAINERTYPE_NORMAL
	db 30, QWILFISH
	db 32, MAREANIE
	db -1 ; end

	; FISHER (24)
	db "RALPH@", TRAINERTYPE_MOVES
	db 35, QWILFISH,   TOXIC, MINIMIZE, SURF, PIN_MISSILE
	db 39, TOXAPEX,    ENDURE, FLAIL, FURY_ATTACK, WATERFALL
	db -1 ; end

	; FISHER (25)
	db "TULLY@", TRAINERTYPE_MOVES
	db 34, TOXAPEX,    SUPERSONIC, RAIN_DANCE, WATERFALL, FURY_ATTACK
	db 34, TOXAPEX,    SUPERSONIC, RAIN_DANCE, WATERFALL, FURY_ATTACK
	db 37, QWILFISH,   ROLLOUT, SURF, PIN_MISSILE, TAKE_DOWN
	db -1 ; end

SwimmerMGroup:
	; SWIMMERM (1)
	db "HAROLD@", TRAINERTYPE_NORMAL
	db 32, REMORAID
	db 30, SEADRA
	db -1 ; end

	; SWIMMERM (2)
	db "ALISTAIR@", TRAINERTYPE_NORMAL
	db 25, SHELLDER
	db 25, TENTACOOL
	db -1 ; end

	; SWIMMERM (3)
	db "CALLUM@", TRAINERTYPE_NORMAL
	db 23, STARYU
	db 25, SQUIRTLE
	db 23, SHELLDER
	db -1 ; end

	; SWIMMERM (4)
	db "GARRY@", TRAINERTYPE_NORMAL
	db 25, SHELLDER
	db 23, HORSEA
	db 23, TENTACOOL
	db -1 ; end

	; SWIMMERM (5)
	db "DEAN@", TRAINERTYPE_NORMAL
	db 22, PSYDUCK
	db 22, AZUMARILL
	db 22, PSYDUCK
	db 23, STARYU
	db 22, TENTACOOL
	db 23, REMORAID
	db -1 ; end

	; SWIMMERM (6)
	db "EUAN@", TRAINERTYPE_NORMAL
	db 22, WARTORTLE
	db 27, QWILFISH
	db -1 ; end

	; SWIMMERM (7)
	db "KEITH@", TRAINERTYPE_NORMAL
	db 24, QUAGSIRE
	db 24, GYARADOS
	db -1 ; end

	; SWIMMERM (8)
	db "BRYAN@", TRAINERTYPE_NORMAL
	db 22, REMORAID
	db 27, KRABBY
	db -1 ; end

	; SWIMMERM (9)
	db "HAL@", TRAINERTYPE_NORMAL
	db 24, SEEL
	db 25, DEWGONG
	db 24, SEEL
	db -1 ; end

	; SWIMMERM (10)
	db "PATON@", TRAINERTYPE_NORMAL
	db 26, PILOSWINE
	db 26, PILOSWINE
	db -1 ; end

	; SWIMMERM (11)
	db "DARYL@", TRAINERTYPE_NORMAL
	db 24, SHELLDER
	db 25, CLOYSTER
	db 24, SHELLDER
	db -1 ; end

	; SWIMMERM (12)
	db "WALTER@", TRAINERTYPE_NORMAL
	db 15, HORSEA
	db 15, HORSEA
	db 20, SEADRA
	db -1 ; end

	; SWIMMERM (13)
	db "TONY@", TRAINERTYPE_NORMAL
	db 13, STARYU
	db 18, STARMIE
	db 16, HORSEA
	db -1 ; end

	; SWIMMERM (14)
	db "JEROME@", TRAINERTYPE_NORMAL
	db 26, SEADRA
	db 28, TENTACOOL
	db 30, TENTACRUEL
	db 28, MAREANIE
	db -1 ; end

	; SWIMMERM (15)
	db "TUCKER@", TRAINERTYPE_NORMAL
	db 30, SHELLDER
	db 34, CLOYSTER
	db -1 ; end

	; SWIMMERM (16)
	db "RICK@", TRAINERTYPE_NORMAL
	db 13, STARYU
	db 18, STARMIE
	db 16, HORSEA
	db -1 ; end

	; SWIMMERM (17)
	db "CAMERON@", TRAINERTYPE_NORMAL
	db 34, MARILL
	db -1 ; end

	; SWIMMERM (18)
	db "SETH@", TRAINERTYPE_NORMAL
	db 29, QUAGSIRE
	db 29, OCTILLERY
	db 32, QUAGSIRE
	db -1 ; end

	; SWIMMERM (19)
	db "JAMES@", TRAINERTYPE_NORMAL
	db 13, STARYU
	db 18, STARMIE
	db 16, HORSEA
	db -1 ; end

	; SWIMMERM (20)
	db "LEWIS@", TRAINERTYPE_NORMAL
	db 13, STARYU
	db 18, STARMIE
	db 16, HORSEA
	db -1 ; end

	; SWIMMERM (21)
	db "PARKER@", TRAINERTYPE_NORMAL
	db 32, HORSEA
	db 32, HORSEA
	db 35, SEADRA
	db -1 ; end

SwimmerFGroup:
	; SWIMMERF (1)
	db "GILLIAN@", TRAINERTYPE_NORMAL
	db 21, SHELLDER
	db 26, STARYU
	db -1 ; end

	; SWIMMERF (2)
	db "NATALIE@", TRAINERTYPE_NORMAL
	db 24, STARYU
	db 24, SHELLDER
	db -1 ; end

	; SWIMMERF (3)
	db "AMY@", TRAINERTYPE_NORMAL
	db 22, PSYDUCK
	db 24, KRABBY
	db 24, PSYDUCK
	db -1 ; end

	; SWIMMERF (4)
	db "CHERYL@", TRAINERTYPE_MOVES
	db 24, PSYDUCK,    SCRATCH, DISABLE, CONFUSION, SCREECH
	db 26, QWILFISH,    TACKLE, POISON_STING, MINIMIZE, WATER_GUN
	db -1 ; end

	; SWIMMERF (5)
	db "VICTORIA@", TRAINERTYPE_NORMAL
	db 22, QWILFISH
	db 26, SEEL
	db -1 ; end

	; SWIMMERF (6)
	db "KARA@", TRAINERTYPE_NORMAL
	db 24, SLOWPOKE
	db 24, STARYU
	db -1 ; end

	; SWIMMERF (7)
	db "HEATHER@", TRAINERTYPE_MOVES
	db 25, HORSEA,     BUBBLE, SMOKESCREEN, LEER, WATER_GUN
	db 25, HORSEA,     DRAGON_RAGE, BUBBLE, SMOKESCREEN, WATER_GUN
	db -1 ; end

	; SWIMMERF (8)
	db "LISA@", TRAINERTYPE_NORMAL
	db 28, CORVIKNGHT
	db -1 ; end

	; SWIMMERF (9)
	db "JILL@", TRAINERTYPE_NORMAL
	db 28, DEWGONG
	db -1 ; end

	; SWIMMERF (10)
	db "MARY@", TRAINERTYPE_NORMAL
	db 20, TOXAPEX
	db -1 ; end

	; SWIMMERF (11)
	db "KATIE@", TRAINERTYPE_NORMAL
	db 33, DEWGONG
	db -1 ; end

	; SWIMMERF (12)
	db "DAWN@", TRAINERTYPE_NORMAL
	db 34, TOXAPEX
	db -1 ; end

	; SWIMMERF (13)
	db "TARA@", TRAINERTYPE_NORMAL
	db 20, TOXAPEX
	db -1 ; end

	; SWIMMERF (14)
	db "NICOLE@", TRAINERTYPE_NORMAL
	db 29, MARILL
	db 29, MARILL
	db 32, LAPRAS
	db -1 ; end

	; SWIMMERF (15)
	db "LORI@", TRAINERTYPE_NORMAL
	db 32, STARMIE
	db 32, STARMIE
	db -1 ; end

	; SWIMMERF (16)
	db "JODY@", TRAINERTYPE_NORMAL
	db 20, TOXAPEX
	db -1 ; end

	; SWIMMERF (17)
	db "NIKKI@", TRAINERTYPE_NORMAL
	db 28, SEEL
	db 28, SEEL
	db 28, SEEL
	db 28, DEWGONG
	db -1 ; end

	; SWIMMERF (18)
	db "DIANA@", TRAINERTYPE_NORMAL
	db 37, GOLDUCK
	db -1 ; end

	; SWIMMERF (19)
	db "BRIANA@", TRAINERTYPE_NORMAL
	db 35, TOXAPEX
	db 35, TOXAPEX
	db -1 ; end

SailorGroup:
	; SAILOR (1)
	db "JOSEPH@", TRAINERTYPE_MOVES
	db 21, QUAGSIRE,    WATER_GUN, TAIL_WHIP, SLAM, NO_MOVE
	db 21, RATICATE,	TAIL_WHIP, QUICK_ATTACK, HYPER_FANG, FOCUS_ENERGY
	db 23, KRABBY,		BUBBLE, LEER, VICEGRIP, STOMP
	db -1 ; end

	; SAILOR (2)
	db "GREG@", TRAINERTYPE_NORMAL
	db 22, TENTACOOL
	db 22, RATICATE
	db -1 ; end

	; SAILOR (3)
	db "CALUM@", TRAINERTYPE_NORMAL
	db 20, KRABBY
	db 24, TENTACOOL
	db -1 ; end

	; SAILOR (4)
	db "GEORGE@", TRAINERTYPE_MOVES
	db 22, RATICATE,   TAIL_WHIP, QUICK_ATTACK, HYPER_FANG, FOCUS_ENERGY
	db 24, KRABBY,     BUBBLEBEAM, LEER, VICEGRIP, STOMP
	db -1 ; end

	; SAILOR (5)
	db "DUNCAN@", TRAINERTYPE_NORMAL
	db 22, RATICATE
	db 22, PSYDUCK
	db 22, GYARADOS
	db -1 ; end

	; SAILOR (6)
	db "JEFF@", TRAINERTYPE_NORMAL
	db 32, RATICATE
	db 32, RATICATE
	db -1 ; end

	; SAILOR (7)
	db "GARRETT@", TRAINERTYPE_NORMAL
	db 34, KINGLER
	db -1 ; end

	; SAILOR (8)
	db "KENNETH@", TRAINERTYPE_NORMAL
	db 28, TIMBURR
	db 28, TIMBURR
	db 28, LEAFEON
	db 28, TIMBURR
	db -1 ; end

	; SAILOR (9)
	db "STANLY@", TRAINERTYPE_NORMAL
	db 31, TIMBURR
	db 33, MACHOKE
	db 26, PSYDUCK
	db -1 ; end

	; SAILOR (10)
	db "JASON@", TRAINERTYPE_NORMAL
	db 19, KRABBY
	db 24, QUAGSIRE
	db -1 ; end

	; SAILOR (11)
	db "HUEY@", TRAINERTYPE_NORMAL
	db 28, LYCANROC
	db 28, LYCANROC
	db -1 ; end

	; SAILOR (12)
	db "HUEY@", TRAINERTYPE_NORMAL
	db 34, LYCANROC
	db 34, LEAFEON
	db -1 ; end

	; SAILOR (13)
	db "HUEY@", TRAINERTYPE_MOVES
	db 38, NOIVERN,   WHIRLPOOL, RAIN_DANCE, BODY_SLAM, PERISH_SONG
	db 38, LEAFEON,  SURF, STRENGTH, ICE_PUNCH, SUBMISSION
	db -1 ; end
	
	; SAILOR (14)
	db "WAYNE@", TRAINERTYPE_NORMAL
	db 31, YAMPER
	db 36, PAWMO
	db -1 ; end

SuperNerdGroup:
	; SUPER_NERD (1)
	db "STAN@", TRAINERTYPE_NORMAL
	db 20, GRIMER
	db -1 ; end

	; SUPER_NERD (2)
	db "DARREN@", TRAINERTYPE_NORMAL
	db 15, GRIMER
	db 15, PORYGON
	db -1 ; end

	; SUPER_NERD (3)
	db "GREGG@", TRAINERTYPE_NORMAL
	db 20, YAMPER
	db 20, YAMPER
	db 20, YAMPER
	db -1 ; end

	; SUPER_NERD (4)
	db "JAY@", TRAINERTYPE_NORMAL
	db 22, KOFFING
	db 22, KOFFING
	db -1 ; end

	; SUPER_NERD (5)
	db "DAVE@", TRAINERTYPE_NORMAL
	db 24, DITTO
	db -1 ; end

	; SUPER_NERD (6)
	db "SAM@", TRAINERTYPE_NORMAL
	db 34, GRIMER
	db 34, MUK
	db -1 ; end

	; SUPER_NERD (7)
	db "TOM@", TRAINERTYPE_NORMAL
	db 32, YAMPER
	db 32, YAMPER
	db 32, YAMPER
	db -1 ; end

	; SUPER_NERD (8)
	db "PAT@", TRAINERTYPE_NORMAL
	db 36, PORYGON
	db -1 ; end

	; SUPER_NERD (9)
	db "SHAWN@", TRAINERTYPE_NORMAL
	db 31, YAMPER
	db 33, MUK
	db 31, YAMPER
	db -1 ; end

	; SUPER_NERD (10)
	db "SEAN@", TRAINERTYPE_NORMAL
	db 12, GRIMER
	db 16, HORSEA
	db 12, SLOWPOKE
	db 14, SLOWPOKE
	db -1 ; end

	; SUPER_NERD (11)
	db "RUSS@", TRAINERTYPE_NORMAL
	db 27, YAMPER
	db 27, YAMPER
	db 27, YAMPER
	db -1 ; end

	; SUPER_NERD (12)
	db "NORTON@", TRAINERTYPE_MOVES
	db 30, PORYGON,    CONVERSION, CONVERSION2, RECOVER, TRI_ATTACK
	db -1 ; end

	; SUPER_NERD (13)
	db "HUGH@", TRAINERTYPE_MOVES
	db 39, SEADRA,     SMOKESCREEN, TWISTER, SURF, WATERFALL
	db -1 ; end

	; SUPER_NERD (14)
	db "EWAN@", TRAINERTYPE_MOVES
	db 26, HORSEA,     BUBBLE, SMOKESCREEN, LEER, WATER_GUN
	db 31, SLOWPOKE,   CURSE, WATER_GUN, CONFUSION, STRENGTH
	db -1 ; end

Rival2Group:
	; RIVAL2 (1)
	db "?@", TRAINERTYPE_MOVES
	db 41, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 42, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 41, BOLTUND,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 43, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 43, ALAKAZAM,   DISABLE, RECOVER, FUTURE_SIGHT, PSYCHIC_M
	db 45, MEOWSCARAD,   RAZOR_LEAF, POISONPOWDER, BODY_SLAM, LIGHT_SCREEN
	db -1 ; end

	; RIVAL2 (2)
	db "?@", TRAINERTYPE_MOVES
	db 41, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 42, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 41, BOLTUND,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 43, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 43, ALAKAZAM,   DISABLE, RECOVER, FUTURE_SIGHT, PSYCHIC_M
	db 45, TYPHLOSION, SMOKESCREEN, QUICK_ATTACK, FLAME_WHEEL, SWIFT
	db -1 ; end

	; RIVAL2 (3)
	db "?@", TRAINERTYPE_MOVES
	db 41, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 42, GOLBAT,     LEECH_LIFE, BITE, CONFUSE_RAY, WING_ATTACK
	db 41, BOLTUND,   THUNDERSHOCK, SONICBOOM, THUNDER_WAVE, SWIFT
	db 43, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 43, ALAKAZAM,   DISABLE, RECOVER, FUTURE_SIGHT, PSYCHIC_M
	db 45, FERALIGATR, RAGE, WATER_GUN, SCARY_FACE, SLASH
	db -1 ; end

	; RIVAL2 (4)
	db "?@", TRAINERTYPE_MOVES
	db 45, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 48, CROBAT,     TOXIC, BITE, CONFUSE_RAY, WING_ATTACK
	db 45, BOLTUND,   THUNDER, SONICBOOM, THUNDER_WAVE, SWIFT
	db 46, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 46, ALAKAZAM,   RECOVER, FUTURE_SIGHT, PSYCHIC_M, REFLECT
	db 50, MEOWSCARAD,   GIGA_DRAIN, BODY_SLAM, LIGHT_SCREEN, SAFEGUARD
	db -1 ; end

	; RIVAL2 (5)
	db "?@", TRAINERTYPE_MOVES
	db 45, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 48, CROBAT,     TOXIC, BITE, CONFUSE_RAY, WING_ATTACK
	db 45, BOLTUND,   THUNDER, SONICBOOM, THUNDER_WAVE, SWIFT
	db 46, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 46, ALAKAZAM,   RECOVER, FUTURE_SIGHT, PSYCHIC_M, REFLECT
	db 50, TYPHLOSION, SMOKESCREEN, QUICK_ATTACK, FIRE_BLAST, SWIFT
	db -1 ; end

	; RIVAL2 (6)
	db "?@", TRAINERTYPE_MOVES
	db 45, SNEASEL,    QUICK_ATTACK, SCREECH, FAINT_ATTACK, FURY_CUTTER
	db 48, CROBAT,     TOXIC, BITE, CONFUSE_RAY, WING_ATTACK
	db 45, BOLTUND,   THUNDER, SONICBOOM, THUNDER_WAVE, SWIFT
	db 46, GENGAR,     MEAN_LOOK, CURSE, SHADOW_BALL, CONFUSE_RAY
	db 46, ALAKAZAM,   RECOVER, FUTURE_SIGHT, PSYCHIC_M, REFLECT
	db 50, FERALIGATR, SURF, RAIN_DANCE, SLASH, SCREECH
	db -1 ; end

GuitaristGroup:
	; GUITARIST (1)
	db "CLYDE@", TRAINERTYPE_NORMAL
	db 34, WHIRLIPEDE
	db -1 ; end

	; GUITARIST (2)
	db "VINCENT@", TRAINERTYPE_NORMAL
	db 27, YAMPER
	db 33, BLITZLE
	db 32, YAMPER
	db 32, YAMPER
	db -1 ; end
	
	; GUITARIST (3)
	db "BRUCE@", TRAINERTYPE_NORMAL
	db 32, LUXIO
	db 32, FLAAFFY
	db 33, LANTURN
	db -1 ; end

	; GUITARIST (4)
	db "RUSSELL@", TRAINERTYPE_NORMAL
	db 32, JOLTEON
	db 32, ARCTOZOLT
	db 33, ZEBSTRIKA
	db -1 ; end

HikerGroup:
	; HIKER (1)
	db "ANTHONY@", TRAINERTYPE_NORMAL
	db 16, GEODUDE
	db 18, MACHAMP
	db -1 ; end

	; HIKER (2)
	db "STEVEN@", TRAINERTYPE_NORMAL
	db  8, ZUBAT
	db  8, TIMBURR
	db  10, GEODUDE
	db -1 ; end

	; HIKER (3)
	db "EDWARD@", TRAINERTYPE_NORMAL
	db 23, RHYHORN
	db 23, SWINUB
	db 23, GEODUDE
	db -1 ; end

	; HIKER (4)
	db "DONALD@", TRAINERTYPE_NORMAL
	db 23, GEODUDE
	db 25, SUDOWOODO
	db -1 ; end

	; HIKER (5)
	db "JAMIE@", TRAINERTYPE_NORMAL
	db 13, GEODUDE
	db 13, PHANPY
	db -1 ; end

	; HIKER (6)
	db "BENJAMIN@", TRAINERTYPE_NORMAL
	db 27, AERODACTYL
	db 27, GRAVELER
	db 29, RHYHORN
	db -1 ; end

	; HIKER (7)
	db "ERIK@", TRAINERTYPE_NORMAL
	db 24, TIMBURR
	db 27, GRAVELER
	db 27, TIMBURR
	db -1 ; end

	; HIKER (8)
	db "MICHAEL@", TRAINERTYPE_NORMAL
	db 25, GEODUDE
	db 25, GRAVELER
	db 25, GOLEM
	db -1 ; end

	; HIKER (9)
	db "PARRY@", TRAINERTYPE_NORMAL
	db 35, ONIX
	db 33, SWINUB
	db -1 ; end

	; HIKER (10)
	db "TIMOTHY@", TRAINERTYPE_MOVES
	db 27, MUDBRAY,    MAGNITUDE, DIG, SAND_ATTACK, SLASH
	db 27, MUDSDALE,    MAGNITUDE, DIG, SAND_ATTACK, SLASH
	db -1 ; end

	; HIKER (11)
	db "KENNETH@", TRAINERTYPE_NORMAL
	db 17, LARVITAR
	db 17, ZUBAT
	db 17, RHYHORN
	db 17, GEODUDE
	db 17, GEODUDE
	db -1 ; end

	; HIKER (12)
	db "ANTHONY@", TRAINERTYPE_NORMAL
	db 25, GRAVELER
	db 27, GRAVELER
	db 29, MACHOKE
	db -1 ; end

	; HIKER (13)
	db "TIM@", TRAINERTYPE_NORMAL
	db 31, GRAVELER
	db 31, GRAVELER
	db 31, GRAVELER
	db -1 ; end

	; HIKER (14)
	db "NOLAND@", TRAINERTYPE_NORMAL
	db 31, SANDSLASH
	db 33, GOLEM
	db -1 ; end

	; HIKER (15)
	db "SIDNEY@", TRAINERTYPE_NORMAL
	db 34, MUDSDALE
	db 32, ONIX
	db -1 ; end

	; HIKER (16)
	db "KENNY@", TRAINERTYPE_NORMAL
	db 27, SANDSLASH
	db 29, GRAVELER
	db 31, GOLEM
	db 29, GRAVELER
	db -1 ; end

	; HIKER (17)
	db "JIM@", TRAINERTYPE_NORMAL
	db 35, MACHAMP
	db -1 ; end

	; HIKER (18)
	db "ROSS@", TRAINERTYPE_NORMAL
	db  8, SANDSHREW
	db 13, ONIX
	db -1 ; end

	; HIKER (19)
	db "PARRY@", TRAINERTYPE_MOVES
	db 35, PILOSWINE,  EARTHQUAKE, BLIZZARD, REST, TAKE_DOWN
	db 35, MUDSDALE,    MAGNITUDE, DIG, MUD_SLAP, SLASH
	db 38, STEELIX,    DIG, IRON_TAIL, SANDSTORM, SLAM
	db -1 ; end

	; HIKER (20)
	db "PARRY@", TRAINERTYPE_NORMAL
	db 29, ONIX
	db -1 ; end

	; HIKER (21)
	db "ANTHONY@", TRAINERTYPE_NORMAL
	db 30, GRAVELER
	db 30, GRAVELER
	db 32, MACHOKE
	db -1 ; end

	; HIKER (22)
	db "ANTHONY@", TRAINERTYPE_MOVES
	db 34, GRAVELER,   MAGNITUDE, SELFDESTRUCT, DEFENSE_CURL, ROLLOUT
	db 36, GOLEM,      MAGNITUDE, SELFDESTRUCT, DEFENSE_CURL, ROLLOUT
	db 34, MACHOKE,    KARATE_CHOP, VITAL_THROW, HEADBUTT, DIG
	db -1 ; end

BikerGroup:
	; BIKER (1)
	db "BENNY@", TRAINERTYPE_NORMAL
	db 20, KOFFING
	db 20, KOFFING
	db 20, KOFFING
	db -1 ; end

	; BIKER (2)
	db "KAZU@", TRAINERTYPE_NORMAL
	db 20, KOFFING
	db 20, KOFFING
	db 20, KOFFING
	db -1 ; end

	; BIKER (3)
	db "DWAYNE@", TRAINERTYPE_NORMAL
	db 27, KOFFING
	db 28, KOFFING
	db 29, KOFFING
	db 30, KOFFING
	db -1 ; end

	; BIKER (4)
	db "HARRIS@", TRAINERTYPE_NORMAL
	db 34, FLAREON
	db -1 ; end

	; BIKER (5)
	db "ZEKE@", TRAINERTYPE_NORMAL
	db 32, KOFFING
	db 32, KOFFING
	db -1 ; end

	; BIKER (6)
	db "CHARLES@", TRAINERTYPE_NORMAL
	db 30, KOFFING
	db 30, CHARMELEON
	db 30, WEEZING
	db -1 ; end

	; BIKER (7)
	db "RILEY@", TRAINERTYPE_NORMAL
	db 34, WEEZING
	db -1 ; end

	; BIKER (8)
	db "JOEL@", TRAINERTYPE_NORMAL
	db 32, MAGMAR
	db 32, MAGMAR
	db -1 ; end

	; BIKER (9)
	db "GLENN@", TRAINERTYPE_NORMAL
	db 28, KOFFING
	db 30, MAGMAR
	db 32, WEEZING
	db -1 ; end

BlaineGroup:
	; BLAINE (1)
	db "BLAINE@", TRAINERTYPE_MOVES
	db 45, MAGCARGO,   CURSE, SMOG, FLAMETHROWER, ROCK_SLIDE
	db 45, MAGMAR,     THUNDERPUNCH, FIRE_PUNCH, SUNNY_DAY, CONFUSE_RAY
	db 50, RAPIDASH,   QUICK_ATTACK, FIRE_SPIN, FURY_ATTACK, FIRE_BLAST
	db -1 ; end

BurglarGroup:
	; BURGLAR (1)
	db "DALE@", TRAINERTYPE_NORMAL
	db 30, CHARMELEON
	db 31, MAGMAR
	db 30, KOFFING
	db -1 ; end

	; BURGLAR (2)
	db "JACK@", TRAINERTYPE_MOVES
	db 32, GROWLITHE,  BITE, EMBER, LEER, TAKE_DOWN
	db 30, KOFFING,    POISON_GAS, TACKLE, SLUDGE, SMOKESCREEN
	db -1 ; end

	; BURGLAR (3)
	db "COREY@", TRAINERTYPE_NORMAL
	db 25, KOFFING
	db 28, MAGMAR
	db 25, KOFFING
	db 30, KOFFING
	db -1 ; end

FirebreatherGroup:
	; FIREBREATHER (1)
	db "OTIS@", TRAINERTYPE_NORMAL
	db 29, MAGMAR
	db 32, WEEZING
	db 29, MAGMAR
	db -1 ; end

	; FIREBREATHER (2)
	db "DICK@", TRAINERTYPE_NORMAL
	db 17, CHARMELEON
	db -1 ; end

	; FIREBREATHER (3)
	db "NED@", TRAINERTYPE_NORMAL
	db 15, KOFFING
	db 16, GROWLITHE
	db 15, KOFFING
	db -1 ; end

	; FIREBREATHER (4)
	db "BURT@", TRAINERTYPE_NORMAL
	db 32, KOFFING
	db 32, SLUGMA
	db -1 ; end

	; FIREBREATHER (5)
	db "GARY@", TRAINERTYPE_NORMAL
	db  8, GROWLITHE
	db  8, KOFFING
	db -1 ; end

	; FIREBREATHER (6)
	db "MATT@", TRAINERTYPE_NORMAL
	db 15, SLUGMA
	db 17, MAGMAR
	db -1 ; end

	; FIREBREATHER (7)
	db "WILL@", TRAINERTYPE_NORMAL
	db  8, CHARMANDER
	db 11, VULPIX
	db -1 ; end

	; FIREBREATHER (8)
	db "LYLE@", TRAINERTYPE_NORMAL
	db 28, KOFFING
	db 31, FLAREON
	db 28, KOFFING
	db -1 ; end

JugglerGroup:
	; JUGGLER (1)
	db "GRAHAM@", TRAINERTYPE_NORMAL
	db 15, DROWZEE
	db 15, GASTLY
	db 15, MAGBY
	db 18, GASTLY
	db -1 ; end

	; JUGGLER (2)
	db "FRITZ@", TRAINERTYPE_NORMAL
	db 29, CAPSAKID
	db 29, MAGMAR
	db 29, MACHOKE
	db -1 ; end

	; JUGGLER (3)
	db "HORTON@", TRAINERTYPE_NORMAL
	db 33, ZEBSTRIKA
	db 33, ZEBSTRIKA
	db 33, ZEBSTRIKA
	db 33, ZEBSTRIKA
	db -1 ; end

	; JUGGLER (4)
	db "KRIS@", TRAINERTYPE_NORMAL
	db 34, ARCTOZOLT
	db 34, AMPHAROS
	db -1 ; end

	; JUGGLER (5)
	db "IRWIN@", TRAINERTYPE_NORMAL
	db 18, BLITZLE
	db 22, BLITZLE
	db 26, BLITZLE
	db 30, ZEBSTRIKA
	db -1 ; end

	; JUGGLER (6)
	db "IRWIN@", TRAINERTYPE_NORMAL
	db 18, BLITZLE
	db 22, BLITZLE
	db 26, BLITZLE
	db 30, ZEBSTRIKA
	db -1 ; end

BlackbeltGroup:
	; BLACKBELT_T (1)
	db "PETER@", TRAINERTYPE_NORMAL
	db 16, LARVITAR
	db 16, CUBONE
	db -1 ; end

	; BLACKBELT_T (2)
	db "XIAN@", TRAINERTYPE_NORMAL
	db 23, MEOWTH
	db 28, PERRSERKER
	db -1 ; end

	; BLACKBELT_T (3)
	db "KENJI@", TRAINERTYPE_MOVES
	db 33, ONIX,       BIND, ROCK_THROW, TOXIC, DIG
	db 38, MACHAMP,    HEADBUTT, SWAGGER, THUNDERPUNCH, VITAL_THROW
	db 33, STEELIX,    EARTHQUAKE, ROCK_THROW, IRON_TAIL, SANDSTORM
	db 36, GULPIN,  DOUBLE_TEAM, HI_JUMP_KICK, MUD_SLAP, SWIFT
	db -1 ; end

	; BLACKBELT_T (4) - REMOVED
	db "LAO@", TRAINERTYPE_MOVES
	db 27, SIGILYPH, COMET_PUNCH, THUNDERPUNCH, ICE_PUNCH, FIRE_PUNCH
	db -1 ; end

	; BLACKBELT_T (5) - REMOVED
	db "NOB@", TRAINERTYPE_MOVES
	db 25, TIMBURR,    LEER, FOCUS_ENERGY, KARATE_CHOP, SEISMIC_TOSS
	db 25, MACHOKE,    LEER, KARATE_CHOP, SEISMIC_TOSS, ROCK_SLIDE
	db -1 ; end

	; BLACKBELT_T (6)
	db "KIYO@", TRAINERTYPE_NORMAL
	db 34, GULPIN
	db 34, SIGILYPH
	db -1 ; end

	; BLACKBELT_T (7) - REMOVED
	db "LUNG@", TRAINERTYPE_NORMAL
	db 23, GREAVARD
	db 23, GREAVARD
	db 25, HOUNDSTONE
	db -1 ; end

	; BLACKBELT_T (8)
	db "KENJI@", TRAINERTYPE_NORMAL
	db 28, MACHOKE
	db -1 ; end

	; BLACKBELT_T (9)
	db "WAI@", TRAINERTYPE_NORMAL
	db 30, MACHOKE
	db 32, MACHOKE
	db 34, MACHOKE
	db -1 ; end

ExecutiveMGroup:
	; EXECUTIVEM (1)
	db "EXECUTIVE@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_MOVES
	db 39, MUK
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db POUND, SLUDGE, MINIMIZE, SCREECH
	db 39, WEEZING
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db POISON_GAS, TACKLE, SLUDGE, SMOKESCREEN
	db 41, HOUNDOOM
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db FIRE_BLAST, SHARPEN, SMOG, BITE
	db -1 ; end

	; EXECUTIVEM (2)
	db "EXECUTIVE@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_MOVES
	db 37, ABSOL
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BITE, SWORDS_DANCE, DOUBLE_TEAM, SLASH
	db 42, GOLBAT
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db BITE, CONFUSE_RAY, WING_ATTACK, TOXIC
	db -1 ; end

	; EXECUTIVEM (3)
	db "EXECUTIVE@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_MOVES
	db 35, PORYGON
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db CONVERSION2, PSYBEAM, RECOVER, TACKLE
	db 36, MAREANIE
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db POISON_STING, PECK, BITE, TOXIC
	db 36, BOMBIRDIER
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db LEER, WING_ATTACK, THIEF, ROCK_THROW
	db 38, RATICATE
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db HYPER_FANG, FOCUS_ENERGY, PURSUIT, SHADOW_BALL
	db 36, GOLBAT
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db SCREECH, BITE, CONFUSE_RAY, WING_ATTACK
	db 36, WEEZING
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db TACKLE, SELFDESTRUCT, SLUDGE, SMOKESCREEN
	db -1 ; end

	; EXECUTIVEM (4)
	db "EXECUTIVE@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_NORMAL
	db 30, GOLBAT
		dw $00fb, $010b, $00f3, $0100, $00c2 ; hp, atk, def, spd, spc
	db 32, RATICATE
		dw $00fb, $010b, $00f3, $0100, $00c2 ; hp, atk, def, spd, spc
	db 30, KOFFING
		dw $00fb, $010b, $00f3, $0100, $00c2 ; hp, atk, def, spd, spc
	db -1 ; end

PsychicGroup:
	; PSYCHIC_T (1)
	db "JORDAN@", TRAINERTYPE_NORMAL
	db 23, ESPEON
	db 26, GIRAFARIG
	db -1 ; end

	; PSYCHIC_T (2)
	db "FRANKLIN@", TRAINERTYPE_NORMAL
	db 37, KADABRA
	db -1 ; end

	; PSYCHIC_T (3)
	db "HERMAN@", TRAINERTYPE_NORMAL
	db 30, RIOLU
	db 30, RIOLU
	db 30, LUCARIO
	db -1 ; end

	; PSYCHIC_T (4)
	db "FIDEL@", TRAINERTYPE_NORMAL
	db 34, XATU
	db -1 ; end

	; PSYCHIC_T (5)
	db "KYLE@", TRAINERTYPE_MOVES
	db 17, WOBBUFFET,  COUNTER, MIRROR_COAT, SAFEGUARD, DESTINY_BOND
	db 22, DROWZEE,    POUND, HYPNOSIS, CONFUSION, DREAM_EATER
	db -1 ; end

	; PSYCHIC_T (6)
	db "LIAM@", TRAINERTYPE_MOVES
	db 21, SIGILYPH,   GUST, HYPNOSIS, PSYWAVE, NO_MOVE
	db 24, SLOWPOKE,   CURSE, BODY_SLAM, WATER_GUN, CONFUSION
	db -1 ; end

	; PSYCHIC_T (7)
	db "NEIL@", TRAINERTYPE_MOVES
	db 18, GIRAFARIG,  TACKLE, GROWL, CONFUSION, STOMP
	db 18, SLOWPOKE,   CURSE, TACKLE, GROWL, WATER_GUN
	db 20, ABRA,       TELEPORT, FIRE_PUNCH, NO_MOVE, NO_MOVE
	db -1 ; end

	; PSYCHIC_T (8)
	db "RONALD@", TRAINERTYPE_DVS | TRAINERTYPE_MOVES
	db 37, XATU
		db $ab, $88
		db NIGHT_SHADE, FUTURE_SIGHT, DRILL_PECK, HIDDEN_POWER
	db 39, KADABRA
		db $98, $88
		db RECOVER, FUTURE_SIGHT, PSYCHIC_M, DYNAMICPUNCH
	db -1 ; end

	; PSYCHIC_T (9)
	db "RICHARD@", TRAINERTYPE_NORMAL
	db 36, ESPEON
	db -1 ; end

	; PSYCHIC_T (10)
	db "GILBERT@", TRAINERTYPE_NORMAL
	db 30, STARMIE
	db 30, RIOLU
	db 34, GIRAFARIG
	db -1 ; end

	; PSYCHIC_T (11)
	db "JARED@", TRAINERTYPE_NORMAL
	db 32, CAPSAKID
	db 32, RIOLU
	db 35, RIOLU
	db -1 ; end

	; PSYCHIC_T (12)
	db "RODNEY@", TRAINERTYPE_NORMAL
	db 29, DROWZEE
	db 33, HYPNO
	db -1 ; end

PicnickerGroup:
	; PICNICKER (1)
	db "LAURA@", TRAINERTYPE_NORMAL
	db  7, BELLSPROUT
	db  11, NIDORAN_F
	db -1 ; end

	; PICNICKER (2)
	db "EMMA@", TRAINERTYPE_NORMAL
	db 12, SMOLIV
	db 12, TANGELA
	db 12, BULBASAUR
	db -1 ; end

	; PICNICKER (3)
	db "LISA@", TRAINERTYPE_MOVES
	db 15, BULBASAUR,  TACKLE, LEECH_SEED, VINE_WHIP, SLEEP_POWDER
	db 20, BELLSPROUT, VINE_WHIP, GROWTH, WRAP, SLEEP_POWDER
	db -1 ; end

	; PICNICKER (4)
	db "KIRSTY@", TRAINERTYPE_MOVES
	db 15, CLEFAIRY,   GROWL, ENCORE, SING, DOUBLESLAP
	db 19, VULPIX,     EMBER, TAIL_WHIP, QUICK_ATTACK, CONFUSE_RAY
	db -1 ; end

	; PICNICKER (5)
	db "CINDY@", TRAINERTYPE_NORMAL
	db 36, NIDOQUEEN
	db -1 ; end

	; PICNICKER (6)
	db "HOPE@", TRAINERTYPE_NORMAL
	db 34, FLAAFFY
	db -1 ; end

	; PICNICKER (7)
	db "SHARON@", TRAINERTYPE_NORMAL
	db 31, FURRET
	db 33, RAPIDASH
	db -1 ; end

	; PICNICKER (8)
	db "DEBRA@", TRAINERTYPE_NORMAL
	db 33, TOXAPEX
	db -1 ; end

	; PICNICKER (9)
	db "GINA@", TRAINERTYPE_NORMAL
	db 14, SMOLIV
	db 14, SMOLIV
	db 17, IVYSAUR
	db -1 ; end

	; PICNICKER (10)
	db "ASHLEY@", TRAINERTYPE_NORMAL
	db 20, NOCTOWL
	db 20, PONYTA
	db -1 ; end

	; PICNICKER (11)
	db "LAURA@", TRAINERTYPE_NORMAL
	db 19, BELLSPROUT
	db 19, NIDORINA
	db -1 ; end

	; PICNICKER (12)
	db "LIZ@", TRAINERTYPE_NORMAL
	db 19, WEEPINBELL
	db 19, NIDORINO
	db 21, NIDOQUEEN
	db -1 ; end

	; PICNICKER (13)
	db "HEIDI@", TRAINERTYPE_NORMAL
	db 32, DOLLIV
	db 32, DOLLIV
	db -1 ; end

	; PICNICKER (14)
	db "EDNA@", TRAINERTYPE_NORMAL
	db 30, NIDORINA
	db 34, LUXRAY
	db -1 ; end

	; PICNICKER (15)
	db "GINA@", TRAINERTYPE_NORMAL
	db 26, DOLLIV
	db 26, DOLLIV
	db 29, IVYSAUR
	db -1 ; end

	; PICNICKER (16)
	db "TIFFANY@", TRAINERTYPE_MOVES
	db 31, CLEFAIRY,   ENCORE, SING, DOUBLESLAP, MINIMIZE
	db -1 ; end

	; PICNICKER (17)
	db "TIFFANY@", TRAINERTYPE_MOVES
	db 37, CLEFAIRY,   ENCORE, DOUBLESLAP, MINIMIZE, METRONOME
	db -1 ; end

	; PICNICKER (18)
	db "ERIN@", TRAINERTYPE_NORMAL
	db 32, PONYTA
	db 32, PONYTA
	db -1 ; end

	; PICNICKER (19)
	db "TANYA@", TRAINERTYPE_NORMAL
	db 37, LUCARIO
	db -1 ; end

	; PICNICKER (20)
	db "REBECCA@", TRAINERTYPE_MOVES
	db 27, TENTACOOL,  SUPERSONIC, CONSTRICT, ACID, BUBBLEBEAM
	db 32, CLEFAIRY,   SING, DOUBLESLAP, MAGICAL_LEAF, METRONOME
	db -1 ; end

	; PICNICKER (21)
	db "ERIN@", TRAINERTYPE_MOVES
	db 36, PONYTA,     DOUBLE_TEAM, STOMP, FIRE_SPIN, SUNNY_DAY
	db 34, LUXRAY,     SWIFT, MUD_SLAP, QUICK_ATTACK, THUNDERBOLT
	db 36, PONYTA,     DOUBLE_TEAM, STOMP, FIRE_SPIN, SUNNY_DAY
	db -1 ; end

	; PICNICKER (22)
	db "LIZ@", TRAINERTYPE_NORMAL
	db 24, WEEPINBELL
	db 26, NIDORINO
	db 26, NIDOQUEEN
	db -1 ; end

	; PICNICKER (23)
	db "LIZ@", TRAINERTYPE_MOVES
	db 30, WEEPINBELL, SLEEP_POWDER, POISONPOWDER, STUN_SPORE, SLUDGE_BOMB
	db 32, NIDOKING,   EARTHQUAKE, DOUBLE_KICK, POISON_STING, IRON_TAIL
	db 32, NIDOQUEEN,  EARTHQUAKE, DOUBLE_KICK, TAIL_WHIP, BODY_SLAM
	db -1 ; end

	; PICNICKER (24)
	db "GINA@", TRAINERTYPE_NORMAL
	db 30, DOLLIV
	db 30, DOLLIV
	db 32, IVYSAUR
	db -1 ; end

	; PICNICKER (25)
	db "GINA@", TRAINERTYPE_MOVES
	db 33, ARBOLIVA,   STUN_SPORE, SUNNY_DAY, LEECH_SEED, COTTON_SPORE
	db 33, ARBOLIVA,   SUNNY_DAY, SLEEP_POWDER, LEECH_SEED, COTTON_SPORE
	db 38, VENUSAUR,   SOLARBEAM, RAZOR_LEAF, HEADBUTT, MUD_SLAP
	db -1 ; end

	; PICNICKER (26)
	db "TIFFANY@", TRAINERTYPE_MOVES
	db 43, CLEFAIRY,   METRONOME, ENCORE, MOONLIGHT, MINIMIZE
	db -1 ; end
	
	; PICNICKER (27)
	db "SARAH@", TRAINERTYPE_NORMAL
	db  11, BULBASAUR
	db  14, ODDISH
	db -1 ; end

CamperGroup:
	; CAMPER (1)
	db "MICHAEL@", TRAINERTYPE_NORMAL
	db   7, SPEAROW
	db  11, NIDORAN_M
	db -1 ; end

	; CAMPER (2)
	db "DANIEL@", TRAINERTYPE_NORMAL
	db 12, CHARMANDER
	db 14, PSYDUCK
	db -1 ; end

	; CAMPER (3)
	db "BRIAN@", TRAINERTYPE_NORMAL
	db 15, GROWLITHE
	db 15, ZUBAT
	db 18, TAUROS
	db -1 ; end

	; CAMPER (4)
	db "IAN@", TRAINERTYPE_NORMAL
	db 17, SANDSHREW
	db 19, MARILL
	db -1 ; end

	; CAMPER (5)
	db "BARRY@", TRAINERTYPE_NORMAL
	db 36, NIDOKING
	db -1 ; end

	; CAMPER (6)
	db "LLOYD@", TRAINERTYPE_NORMAL
	db 34, NIDOKING
	db -1 ; end

	; CAMPER (7)
	db "DEAN@", TRAINERTYPE_NORMAL
	db 33, GOLDUCK
	db 31, SANDSLASH
	db -1 ; end

	; CAMPER (8)
	db "SID@", TRAINERTYPE_NORMAL
	db 32, MUDSDALE
	db 29, HOUNDSTONE
	db 29, LEAFEON
	db -1 ; end

	; CAMPER (9)
	db "JOHN@", TRAINERTYPE_NORMAL
	db 12, ROCKRUFF
	db -1 ; end

	; CAMPER (10)
	db "CRAIG@", TRAINERTYPE_NORMAL
	db 10, GEODUDE
	db 10, LARVITAR
	db -1 ; end

	; CAMPER (11)
	db "MARC@", TRAINERTYPE_NORMAL
	db 16, GROWLITHE
	db 21, GROWLITHE
	db -1 ; end

	; CAMPER (12)
	db "TODD@", TRAINERTYPE_NORMAL
	db 17, GEODUDE
	db 17, GEODUDE
	db 23, PSYDUCK
	db -1 ; end

	; CAMPER (13)
	db "TODD@", TRAINERTYPE_NORMAL
	db 23, GEODUDE
	db 23, GEODUDE
	db 26, PSYDUCK
	db -1 ; end

	; CAMPER (14)
	db "THOMAS@", TRAINERTYPE_NORMAL
	db 33, GRAVELER
	db 36, GRAVELER
	db 40, GOLBAT
	db 42, GOLDUCK
	db -1 ; end

	; CAMPER (15)
	db "LEROY@", TRAINERTYPE_NORMAL
	db 33, GRAVELER
	db 36, GRAVELER
	db 40, GOLBAT
	db 42, GOLDUCK
	db -1 ; end

	; CAMPER (16)
	db "DAVID@", TRAINERTYPE_NORMAL
	db 33, GRAVELER
	db 36, GRAVELER
	db 40, GOLBAT
	db 42, GOLDUCK
	db -1 ; end

	; CAMPER (17)
	db "JOHN@", TRAINERTYPE_NORMAL
	db 33, GRAVELER
	db 36, GRAVELER
	db 40, GOLBAT
	db 42, GOLDUCK
	db -1 ; end

	; CAMPER (18)
	db "JERRY@", TRAINERTYPE_NORMAL
	db 37, SANDSLASH
	db -1 ; end

	; CAMPER (19)
	db "MOHAMMED@", TRAINERTYPE_NORMAL
	db 29, AIPOM
	db 29, SANDSLASH
	db 31, GOLBAT
	db -1 ; end

	; CAMPER (20)
	db "TODD@", TRAINERTYPE_NORMAL
	db 30, GRAVELER
	db 30, GRAVELER
	db 30, SLUGMA
	db 32, PSYDUCK
	db -1 ; end

	; CAMPER (21)
	db "TODD@", TRAINERTYPE_MOVES
	db 33, GRAVELER,   SELFDESTRUCT, ROCK_THROW, DEFENSE_CURL, MAGNITUDE
	db 33, GRAVELER,   SELFDESTRUCT, ROCK_THROW, DEFENSE_CURL, MAGNITUDE
	db 36, MAGCARGO,   ROCK_THROW, DEFENSE_CURL, AMNESIA, FLAMETHROWER
	db 34, GOLDUCK,    DISABLE, PSYCHIC_M, SURF, PSYCH_UP
	db -1 ; end

	; CAMPER (22)
	db "QUENTIN@", TRAINERTYPE_NORMAL
	db 30, FEAROW
	db 30, HOUNDSTONE
	db 30, TAUROS
	db -1 ; end

ExecutiveFGroup:
	; EXECUTIVEF (1)
	db "EXECUTIVE@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_MOVES
	db 38, HOUNDOOM
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db EMBER, SHARPEN, SMOG, BITE
	db 38, VILEPLUME
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db ABSORB, SLEEP_POWDER, ACID, MOONLIGHT
	db 38, SUDOWOODO
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db CURSE, LOW_KICK, ROCK_SLIDE, FAINT_ATTACK
	db -1 ; end

	; EXECUTIVEF (2)
	db "EXECUTIVE@", TRAINERTYPE_STAT_EXP | TRAINERTYPE_MOVES
	db 31, HOUNDOOM
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db EMBER, SHARPEN, SMOG, BITE
	db 31, VILEPLUME
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db ABSORB, STUN_SPORE, SLEEP_POWDER, ACID
	db 33, SUDOWOODO
		dw $00fb, $010b, $00f3, $00fe, $00c2 ; hp, atk, def, spd, spc
		db MIMIC, FLAIL, LOW_KICK, ROCK_SLIDE
	db -1 ; end

SageGroup:
	; SAGE (1)
	db "YOSHITAKA@", TRAINERTYPE_NORMAL
	db  5, NIDORAN_M
	db  5, RATTATA
	db  5, BELLSPROUT
	db -1 ; end

	; SAGE (2)
	db "NASIR@", TRAINERTYPE_NORMAL
	db  5, SPEAROW
	db  5, EEVEE
	db  5, BELLSPROUT
	db -1 ; end

	; SAGE (3)
	db "NOBUO@", TRAINERTYPE_NORMAL
	db  8, BELLSPROUT
	db -1 ; end

	; SAGE (4)
	db "KIYOSHI@", TRAINERTYPE_NORMAL
	db  9, BELLSPROUT
	db  9, HOOTHOOT
	db -1 ; end

	; SAGE (5) - Cianwood Gym
	db "KATSUHISA@", TRAINERTYPE_NORMAL
	db 23, MEOWTH
	db 28, SKARMORY
	db -1 ; end

	; SAGE (6)
	db "PING@", TRAINERTYPE_NORMAL
	db 16, GASTLY
	db 16, GASTLY
	db 16, GASTLY
	db 16, GASTLY
	db 16, GASTLY
	db -1 ; end

	; SAGE (7)
	db "KENJI@", TRAINERTYPE_NORMAL
	db  5, RATTATA
	db  5, SENTRET
	db  5, BELLSPROUT
	db -1 ; end

	; SAGE (8)
	db "KEN@", TRAINERTYPE_NORMAL
	db  8, BELLSPROUT
	db -1 ; end

	; SAGE (9)
	db "KOICHI@", TRAINERTYPE_NORMAL
	db  7, RATTATA
	db  7, BELLSPROUT
	db 10, HOOTHOOT
	db -1 ; end

	; SAGE (10)
	db "AKITOSHI@", TRAINERTYPE_NORMAL
	db 39, NOCTOWL
	db 39, FLAREON
	db -1 ; end

	; SAGE (11)
	db "HIROMICHI@", TRAINERTYPE_NORMAL
	db 39, NOCTOWL
	db 39, JOLTEON
	db -1 ; end

	; SAGE (12)
	db "HIRONOBU@", TRAINERTYPE_NORMAL
	db 39, NOCTOWL
	db 39, VAPOREON
	db -1 ; end

MediumGroup:
	; MEDIUM (1)
	db "MARTHA@", TRAINERTYPE_NORMAL
	db 18, GASTLY
	db 20, HAUNTER
	db 20, GASTLY
	db -1 ; end

	; MEDIUM (2)
	db "GRACE@", TRAINERTYPE_NORMAL
	db 20, HAUNTER
	db 20, HAUNTER
	db -1 ; end

	; MEDIUM (3)
	db "BETHANY@", TRAINERTYPE_NORMAL
	db 25, HAUNTER
	db -1 ; end

	; MEDIUM (4)
	db "MARGRET@", TRAINERTYPE_NORMAL
	db 25, HAUNTER
	db -1 ; end

	; MEDIUM (5)
	db "ETHEL@", TRAINERTYPE_NORMAL
	db 25, HAUNTER
	db -1 ; end

	; MEDIUM (6)
	db "REBECCA@", TRAINERTYPE_NORMAL
	db 35, DROWZEE
	db 35, HYPNO
	db -1 ; end

	; MEDIUM (7)
	db "DORIS@", TRAINERTYPE_NORMAL
	db 34, SLOWPOKE
	db 36, SLOWBRO
	db -1 ; end

BoarderGroup:
	; BOARDER (1)
	db "MARTYN@", TRAINERTYPE_NORMAL
	db 25, SNEASEL
	db 25, FROSMOTH
	db -1 ; end

	; BOARDER (2)
	db "BRAD@", TRAINERTYPE_NORMAL
	db 26, SWINUB
	db 26, SWINUB
	db -1 ; end

	; BOARDER (3)
	db "DOUGLAS@", TRAINERTYPE_NORMAL
	db 24, SHELLDER
	db 25, CLOYSTER
	db 24, SHELLDER
	db -1 ; end

PokefanMGroup:
	; POKEFANM (1)
	db "GAVIN@", TRAINERTYPE_ITEM
	db 15, DELIBIRD,   BERRY
	db 19, CHARMELEON, BERRY
	db -1 ; end

	; POKEFANM (2)
	db "ANTHONY@", TRAINERTYPE_ITEM
	db 19, IVYSAUR,   BERRY
	db 21, QUAGSIRE,  BERRY
	db -1 ; end

	; POKEFANM (3)
	db "ROBERT@", TRAINERTYPE_ITEM
	db 33, QUAGSIRE,   BERRY
	db -1 ; end

	; POKEFANM (4)
	db "JOSHUA@", TRAINERTYPE_ITEM
	db 23, LUXIO,    BERRY
	db 23, LUXIO,    BERRY
	db 23, LUXIO,    BERRY
	db 23, LUXIO,    BERRY
	db 23, LUXIO,    BERRY
	db 23, LUXIO,    BERRY
	db -1 ; end

	; POKEFANM (5)
	db "CARTER@", TRAINERTYPE_ITEM
	db 29, BULBASAUR,  BERRY
	db 29, CHARMANDER, BERRY
	db 29, SQUIRTLE,   BERRY
	db -1 ; end

	; POKEFANM (6)
	db "TREVOR@", TRAINERTYPE_ITEM
	db 33, PSYDUCK,    BERRY
	db -1 ; end

	; POKEFANM (7)
	db "ALAN@", TRAINERTYPE_ITEM
	db 12, TEDDIURSA,  BERRY
	db 13, BULBASAUR,  BERRY
	db -1 ; end

	; POKEFANM (8)
	db "JEREMY@", TRAINERTYPE_ITEM
	db 28, MEOWTH,     BERRY
	db 28, MEOWTH,     BERRY
	db 28, MEOWTH,     BERRY
	db -1 ; end

	; POKEFANM (9)
	db "COLIN@", TRAINERTYPE_ITEM
	db 32, DELIBIRD,   BERRY
	db -1 ; end

	; POKEFANM (10)
	db "DEREK@", TRAINERTYPE_ITEM
	db 19, LUXIO,    BERRY
	db -1 ; end

	; POKEFANM (11)
	db "DEREK@", TRAINERTYPE_ITEM
	db 36, LUXIO,    BERRY
	db -1 ; end

	; POKEFANM (12)
	db "ALEX@", TRAINERTYPE_ITEM
	db 29, NIDOKING,   BERRY
	db 29, SLOWKING,   BERRY
	db 29, TOXAPEX,    BERRY
	db -1 ; end

	; POKEFANM (13)
	db "REX@", TRAINERTYPE_ITEM
	db 35, PHANPY,     BERRY
	db -1 ; end

	; POKEFANM (14)
	db "ALLAN@", TRAINERTYPE_ITEM
	db 35, TEDDIURSA,  BERRY
	db -1 ; end

KimonoGirlGroup:
	; KIMONO_GIRL (1)
	db "KANAKO@", TRAINERTYPE_MOVES
	db 22, LEAFEON,    TACKLE, GROWL, SAND_ATTACK, RAZOR_LEAF
	db -1 ; end

	; KIMONO_GIRL (2)
	db "RYOKO@", TRAINERTYPE_MOVES
	db 22, FLAREON,    TACKLE, GROWL, SAND_ATTACK, EMBER
	db -1 ; end

	; KIMONO_GIRL (3)
	db "YUKIKO@", TRAINERTYPE_MOVES
	db 22, ESPEON,     TACKLE, GROWL, SAND_ATTACK, CONFUSION
	db -1 ; end

	; KIMONO_GIRL (4)
	db "AYAKO@", TRAINERTYPE_MOVES
	db 22, UMBREON,    TACKLE, GROWL, SAND_ATTACK, PURSUIT
	db -1 ; end

	; KIMONO_GIRL (5)
	db "KAZUKO@", TRAINERTYPE_MOVES
	db 22, VAPOREON,   TACKLE, GROWL, SAND_ATTACK, WATER_GUN
	db -1 ; end

	; KIMONO_GIRL (6)
	db "HIROMI@", TRAINERTYPE_MOVES
	db 22, JOLTEON,    TACKLE, GROWL, SAND_ATTACK, THUNDERSHOCK
	db -1 ; end

TwinsGroup:
	; TWINS (1)
	db "AMY & MAY@", TRAINERTYPE_NORMAL
	db 10, ARCTOZOLT
	db 10, LEDYBA
	db -1 ; end

	; TWINS (2)
	db "ANN & ANNE@", TRAINERTYPE_MOVES
	db 21, CLEFAIRY,   SING, DOUBLESLAP, MINIMIZE, METRONOME
	db 21, JIGGLYPUFF, SING, DEFENSE_CURL, POUND, ROLLOUT
	db -1 ; end

	; TWINS (3)
	db "ANN & ANNE@", TRAINERTYPE_MOVES
	db 21, JIGGLYPUFF, SING, DEFENSE_CURL, POUND, ROLLOUT
	db 21, CLEFAIRY,   SING, DOUBLESLAP, MINIMIZE, METRONOME
	db -1 ; end

	; TWINS (4)
	db "AMY & MAY@", TRAINERTYPE_NORMAL
	db 10, LEDYBA
	db 10, ARCTOZOLT
	db -1 ; end

	; TWINS (5)
	db "JO & ZOE@", TRAINERTYPE_NORMAL
	db 35, VICTREEBEL
	db 35, VILEPLUME
	db -1 ; end

	; TWINS (6)
	db "JO & ZOE@", TRAINERTYPE_NORMAL
	db 35, VILEPLUME
	db 35, VICTREEBEL
	db -1 ; end

	; TWINS (7)
	db "MEG & PEG@", TRAINERTYPE_NORMAL
	db 31, TEDDIURSA
	db 31, PHANPY
	db -1 ; end

	; TWINS (8)
	db "MEG & PEG@", TRAINERTYPE_NORMAL
	db 31, PHANPY
	db 31, TEDDIURSA
	db -1 ; end

	; TWINS (9)
	db "LEA & PIA@", TRAINERTYPE_MOVES
	db 35, DRATINI,    THUNDER_WAVE, TWISTER, FLAMETHROWER, HEADBUTT
	db 35, DRATINI,    THUNDER_WAVE, TWISTER, ICE_BEAM, HEADBUTT
	db -1 ; end

	; TWINS (10)
	db "LEA & PIA@", TRAINERTYPE_MOVES
	db 38, DRATINI,    THUNDER_WAVE, TWISTER, ICE_BEAM, HEADBUTT
	db 38, DRATINI,    THUNDER_WAVE, TWISTER, FLAMETHROWER, HEADBUTT
	db -1 ; end

PokefanFGroup:
	; POKEFANF (1)
	db "MICHELLE@", TRAINERTYPE_ITEM
	db 15, MARILL,     BERRY
	db 19, MEOWTH,     BERRY
	db -1 ; end

	; POKEFANF (2)
	db "KAREN@", TRAINERTYPE_ITEM
	db 19, AZUMARILL,  BERRY
	db 21, AIPOM,      BERRY
	db -1 ; end

	; POKEFANF (3)
	db "BEVERLY@", TRAINERTYPE_ITEM
	db 18, MINCCINO,   BERRY
	db -1 ; end

	; POKEFANF (5)
	db "GEORGIA@", TRAINERTYPE_ITEM
	db 23, SENTRET,    BERRY
	db 23, SENTRET,    BERRY
	db 23, SENTRET,    BERRY
	db 28, FURRET,     BERRY
	db 23, SENTRET,    BERRY
	db -1 ; end

	; POKEFANF (6)
	db "GEMMA@", TRAINERTYPE_ITEM
	db 19, AZUMARILL,  BERRY
	db 20, MEOWTH,     BERRY
	db -1 ; end

RedGroup:
	; RED (1)
	db "RED@", TRAINERTYPE_MOVES
	db 81, LUXIO,    CHARM, QUICK_ATTACK, THUNDERBOLT, THUNDER
	db 73, ESPEON,     MUD_SLAP, REFLECT, SWIFT, PSYCHIC_M
	db 75, SNORLAX,    AMNESIA, SNORE, REST, BODY_SLAM
	db 77, VENUSAUR,   SUNNY_DAY, GIGA_DRAIN, SYNTHESIS, SOLARBEAM
	db 77, CHARIZARD,  FLAMETHROWER, WING_ATTACK, SLASH, FIRE_SPIN
	db 77, BLASTOISE,  RAIN_DANCE, SURF, BLIZZARD, WHIRLPOOL
	db -1 ; end

BlueGroup:
	; BLUE (1)
	db "BLUE@", TRAINERTYPE_MOVES
	db 56, TALONFLAME,    QUICK_ATTACK, WHIRLWIND, WING_ATTACK, MIRROR_MOVE
	db 54, ALAKAZAM,   DISABLE, RECOVER, PSYCHIC_M, REFLECT
	db 56, RHYDON,     FURY_ATTACK, SANDSTORM, ROCK_SLIDE, EARTHQUAKE
	db 58, GYARADOS,   TWISTER, HYDRO_PUMP, RAIN_DANCE, HYPER_BEAM
	db 58, LUCARIO,  SUNNY_DAY, LEECH_SEED, EGG_BOMB, SOLARBEAM
	db 58, ARCANINE,   ROAR, SWIFT, FLAMETHROWER, EXTREMESPEED
	db -1 ; end

OfficerGroup:
	; OFFICER (1)
	db "THOMAS@", TRAINERTYPE_NORMAL
	db 12, GROWLITHE
	db 17, GROWLITHE
	db -1 ; end

	; OFFICER (2)
	db "GORDON@", TRAINERTYPE_NORMAL
	db 18, RIOLU
	db 18, GROWLITHE
	db -1 ; end

GruntFGroup:
	; GRUNTF (1)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 9, ZUBAT
	db 11, DROWZEE
	db -1 ; end

	; GRUNTF (2)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, SNEASEL
	db 32, PERRSERKER
	db -1 ; end

	; GRUNTF (3)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 31, TENTACRUEL
	db 31, GLOOM
	db -1 ; end

	; GRUNTF (4)
	db "GRUNT@", TRAINERTYPE_NORMAL
	db 30, SNEASEL
	db 30, SNEASEL
	db 30, MAGMAR
	db 30, GLOOM
	db -1 ; end

	; GRUNTF (5)
	db "GRUNT@", TRAINERTYPE_MOVES
	db 27, MEOWTH,     SCRATCH, GROWL, METAL_CLAW, PAY_DAY
	db 27, GLOOM,      ABSORB, STUN_SPORE, SLEEP_POWDER, ACID
	db -1 ; end
	
	;GRUNTF (6 - NEW)
	db "AMANDA@", TRAINERTYPE_NORMAL
	db 20, BOMBIRDIER
	db 20, UMBREON
	db -1 ; end
	
	;GRUNTF (7 - NEW)
	db "SAMANTHA@", TRAINERTYPE_NORMAL
	db 18, BOMBIRDIER
	db 20, BOMBIRDIER
	db 20, BOMBIRDIER
	db -1 ; end

MysticalmanGroup:
	; MYSTICALMAN (1)
	db "EUSINE@", TRAINERTYPE_MOVES
	db 26, AZUMARILL,  DEFENSE_CURL, SURF, ROLLOUT, BUBBLEBEAM
	db 26, BLITZLE,    QUICK_ATTACK, TAIL_WHIP, THUNDER_WAVE, NO_MOVE
	db 28, ARCTOZOLT,  BLIZZARD, THUNDERPUNCH, ANCIENTPOWER, WING_ATTACK
	db -1 ; end
